...
static bf_pltfm_cp2112_device_ctx_t *tfn_i2c_hdl = NULL;
...
static int bf_tofino_i2c_init(int chip_id) {
  ...
  tfn_i2c_hdl = bf_pltfm_cp2112_get_handle(CP2112_ID_1);
  if (!tfn_i2c_hdl) {
    TOFINO_PORTING_LOG_ERR("error in bf_tofino_slave_i2c_init\n");
    return -1;
  }
  bit = (1 << MAV_PCA9535_MISC);
  rc = bf_pltfm_cp2112_write_byte(tfn_i2c_hdl, I2C_ADDR_COMM_PCA9548, bit, 100);
...
static int bf_tofino_i2c_deinit(int chip_id) {
  ...
  rc =
      bf_pltfm_cp2112_write_byte(tfn_i2c_hdl, I2C_ADDR_COMM_PCA9548, 0x00, 100);
  if (rc != BF_PLTFM_SUCCESS) {
    TOFINO_PORTING_LOG_ERR("error in setting MISC Channel\n");
    return -1;
  }
...
int bf_tofino_dir_i2c_rd(int chip_id,
  ...
  return (raw_read(upper, I2C_ADDR_TOFINO, &out[0], 5, (uint8_t *)data, 4));
}
...
int bf_tofino_dir_i2c_wr(int chip_id,
  ...
  return (raw_write(upper, I2C_ADDR_TOFINO, &out[0], 9));
}
...
int tofino_porting_spi_reg_wr(int dev_id, uint32_t reg, uint32_t data) {
  return (bf_tofino_dir_i2c_wr(dev_id, 0, reg, data));
}

int tofino_porting_spi_reg_rd(int dev_id, uint32_t reg, uint32_t *data) {
  return (bf_tofino_dir_i2c_rd(dev_id, 0, reg, data));
}
