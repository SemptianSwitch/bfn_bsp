...
* read_from_dv_socket
*********************************************************************/
static int read_from_dv_socket(int sock, uint8_t *buf, int len) {
...
* write_to_dv_socket
*********************************************************************/
static void write_to_dv_socket(int sock, uint8_t *buf, int len) {
...
* write_reg_chnl
*********************************************************************/
static void write_dv_reg_chnl(uint8_t *buf, int len) {
...
* read_reg_chnl
*********************************************************************/
static int read_dv_reg_chnl(uint8_t *buf, int len) {
  return read_from_dv_socket(dv_reg_chnl, buf, len);
}

/*********************************************************************
* create_server
*********************************************************************/
static int create_tcl_server_socket(int listen_port) {
  int client_sock, c;
...
static void tcl_server_loop(void) {
  ...
  while (1) {
    if (g_needs_reconnect) return;

    better_be_positive = read_dv_reg_chnl((unsigned char *)msg, len);
    ...
    if (msg->typ == pcie_op_rd) {  // read
      ...
      if (g_debug_mode) {
        printf("Tcl Rd: asic=%d: addr=%" PRIx64 " = %08x\n",
               msg->asic,
               msg->addr,
               msg->value);
      }
      ...
      if (g_debug_mode) {
        printf("Tcl Wr: asic=%d: addr=%" PRIx64 " = %08x\n",
               msg->asic,
               msg->addr,
               msg->value);
      }
...
static void tcl_server_run(void) {
  ...
  while (1) {
    ...
    dv_reg_chnl = create_tcl_server_socket(DV_REG_CHNL_PORT_DEFAULT);
...
void *tcl_server_init(void *arg) {
  ...
  pthread_cleanup_push(tcl_server_exit_cleanup, "TCL-server Cleanup handler");
  tcl_server_run();
