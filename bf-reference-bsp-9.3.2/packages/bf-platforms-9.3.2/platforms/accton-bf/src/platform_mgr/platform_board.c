...
pltfm_bd_map_t *platform_pltfm_bd_map_get(int *rows) {
  ...
  *rows = ROWS(pltfm_board_maps);
  for (idx = 0; idx < ROWS(pltfm_board_maps); idx++) {
...
int platform_name_get_str(char *name, size_t name_size) {
  ...
  switch (bd_id) {
    case BF_PLTFM_BD_ID_MAVERICKS_P0A:
      snprintf(name, name_size, "Board type: MAVERICKS_P0A");
      break;
    case BF_PLTFM_BD_ID_MAVERICKS_P0B:
      snprintf(name, name_size, "Board type: MAVERICKS_P0B");
      break;
    case BF_PLTFM_BD_ID_MAVERICKS_P0C:
      snprintf(name, name_size, "Board type: MAVERICKS_P0C");
      break;
    case BF_PLTFM_BD_ID_MONTARA_P0A:
      snprintf(name, name_size, "Board type: MONTARA_P0A");
      break;
    case BF_PLTFM_BD_ID_MONTARA_P0B:
      snprintf(name, name_size, "Board type: MONTARA_P0B");
      break;
    case BF_PLTFM_BD_ID_MAVERICKS_P0B_EMU:
      snprintf(name, name_size, "Board type: MAVERICKS_P0B_EMU");
      break;
    case BF_PLTFM_BD_ID_MONTARA_P0C:
      snprintf(name, name_size, "Board type: MONTARA_P0C");
      break;
    default:
...
bf_pltfm_status_t platform_port_mac_addr_get(
    bf_pltfm_port_info_t *port_info, uint8_t *mac_addr) {
