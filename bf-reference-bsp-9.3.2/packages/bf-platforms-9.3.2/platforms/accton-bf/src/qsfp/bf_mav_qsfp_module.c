...
static bf_pltfm_cp2112_device_ctx_t *qsfp_hndl[CP2112_ID_MAX] = {NULL, NULL};
...
int bf_pltfm_qsfp_init(void *arg) {
  ...
  qsfp_hndl[CP2112_ID_1] = bf_pltfm_cp2112_get_handle(CP2112_ID_1);
  ...
  if ((board_id == BF_PLTFM_BD_ID_MAVERICKS_P0B) ||
    ...
    qsfp_hndl[CP2112_ID_2] = bf_pltfm_cp2112_get_handle(CP2112_ID_2);
    if (!qsfp_hndl[CP2112_ID_2]) {
      LOG_ERROR("error getting lower cp2112 handle in %s\n", __func__);
      return -1;
    }
  }
  if (bf_pltfm_init_cp2112_qsfp_bus(qsfp_hndl[CP2112_ID_1],
 ...
 *  <module> is 1 based.
 */
static int mav_qsfp_param_get(unsigned int module,
  ...
  if ((board_id == BF_PLTFM_BD_ID_MONTARA_P0B) ||
    ...
    if (module < 1 || module > (BF_MAV_SUB_PORT_CNT + 1)) {
      return -1;
    }
    /* make module zero based */
    module -= 1;
    if (module < BF_MAV_SUB_PORT_CNT) { /* upper cp2112 subsystem */
      ...
      *sub_module = BF_MAV_SUB_PORT_CNT;
             ...
             (board_id == BF_PLTFM_BD_ID_MAVERICKS_P0A) ||
             (board_id == BF_PLTFM_BD_ID_MAVERICKS_P0C)) {
    if (module < 1 || module > (BF_MAV_SUB_PORT_CNT * 2 + 1)) {
      return -1;
    }
    /* make module zero based */
    module -= 1;
    if (module < BF_MAV_SUB_PORT_CNT) { /* upper cp2112 subsystem */
      *handle = qsfp_hndl[CP2112_ID_1];
      *sub_module = module;
    } else if (module < (BF_MAV_SUB_PORT_CNT * 2)) { /* lower cp2112 */
...
bool bf_pltfm_detect_qsfp(unsigned int module) {
  ...
  rc = bf_mav_qsfp_sub_module_read(hndl, sub_module, ADDR_QSFP_I2C, 0, 1, &val);
...
int bf_pltfm_qsfp_read_module(unsigned int module,
  ...
  while (len >= MAX_MAV_QSFP_I2C_LEN) {
    rc |= bf_mav_qsfp_sub_module_read(
        hndl, sub_module, ADDR_QSFP_I2C, offset, MAX_MAV_QSFP_I2C_LEN, buf);
  ...
  if (len) {
    rc |= bf_mav_qsfp_sub_module_read(
        hndl, sub_module, ADDR_QSFP_I2C, offset, len, buf);
  }
...
int bf_pltfm_qsfp_read_reg(unsigned int module,
  ...
  MAV_QSFP_PAGE_LOCK;
  /* set the page */
  if (bf_pltfm_qsfp_write_module(module, 127, 1, &page)) {
    err = -1;
    goto read_reg_end;
  }
  /* read the register */
  if (bf_pltfm_qsfp_read_module(module, offset, 1, val)) {
    err = -1;
    goto read_reg_end;
  }
  /* reset the page */
...
int bf_pltfm_qsfp_write_reg(unsigned int module,
  ...
  /* read the register */
  if (bf_pltfm_qsfp_write_module(module, offset, 1, &val)) {
    err = -1;
    goto write_reg_end;
  }
  /* reset the page */
 ...
 *   0 on success and -1 in error
 */
int bf_pltfm_qsfp_syscpld_read(bf_pltfm_cp2112_device_ctx_t *hndl,
                               int reg,
                               uint8_t *val) {
  uint8_t wr_val;
  ...
  return (bf_pltfm_cp2112_write_read_unsafe(
      hndl, BF_MAV_SYSCPLD_I2C_ADDR, &wr_val, val, 1, 1, 100));
}
 ...
 *   0 on success and -1 in error
 */
static int bf_pltfm_qsfp_syscpld_write(bf_pltfm_cp2112_device_ctx_t *hndl,
                                       int reg,
                                       uint8_t val) {
  uint8_t wr_val[2];
  ...
  wr_val[0] = reg;
  wr_val[1] = val;
  return (bf_pltfm_cp2112_write(hndl, BF_MAV_SYSCPLD_I2C_ADDR, wr_val, 2, 100));
}
...
int bf_pltfm_qsfp_module_reset(int module, bool reset) {
  ...
  if (sub_module > BF_MAV_SUB_PORT_CNT) {
    return -1;
  }
  if (sub_module == BF_MAV_SUB_PORT_CNT) {
    return (bf_pltfm_set_cpu_module_reset(hndl, reset));
  }

  rc = bf_pltfm_qsfp_syscpld_read(
      hndl, QSFP_SUBMODULE_RESET_REG(sub_module), &val);
  if (rc) {
    return -1;
  }
  if (reset) {
    /* reset control bit is active low */
    val &= ~(1 << QSFP_SUBMODULE_RESET_BIT(sub_module));
  } else {
    val |= (1 << QSFP_SUBMODULE_RESET_BIT(sub_module));
    val |= QSFP_SUBMODULE_RESET_BIT(sub_module);
  }
  rc = bf_pltfm_qsfp_syscpld_write(
      hndl, QSFP_SUBMODULE_RESET_REG(sub_module), val);
  if (rc) {
    return -1;
  }
...
int bf_pltfm_qsfp_get_presence_mask(uint32_t *port_1_32_pres,
  ...
  rc = bf_pltfm_get_sub_module_pres(hndl, port_1_32_pres);

  if (bf_bd_is_this_port_internal(33, 0)) {
    /*
     * Based on this mask, qsfp-scan will proceed. so mark this as absent
     */
    *port_cpu_pres = 0xFFFFFFFFUL;
    /* Return is ok, because on Montara-P0C this is the last port. */
    return rc;
  }

  rc |= bf_pltfm_get_cpu_module_pres(hndl, port_cpu_pres);
  if (rc) {
    return rc;
  }
  ...
  rc = bf_pltfm_get_sub_module_pres(hndl, port_32_64_pres);
...
void bf_pltfm_qsfp_get_int_mask(uint32_t *port_1_32_ints,
  ...
  bf_pltfm_get_sub_module_int(hndl, port_1_32_ints);
  ...
  bf_pltfm_get_sub_module_int(hndl, port_32_64_ints);
}
...
int bf_pltfm_qsfp_get_lpmode_mask(uint32_t *port_1_32_lpm,
  ...
  bf_pltfm_get_sub_module_lpmode(hndl, port_1_32_lpm);
  bf_pltfm_get_cpu_module_lpmode(hndl, port_cpu_lpm);
  ...
  rc = bf_pltfm_get_sub_module_lpmode(hndl, port_32_64_lpm);
  return rc;
}
...
int bf_pltfm_qsfp_set_lpmode(int port, bool lpmode) {
  ...
  /* get the cp2112 for lower numbered ports */
  rc = mav_qsfp_param_get(port, &sub_module, &hndl);
  if (rc) {
    return -1;
  }
  if (sub_module == BF_MAV_SUB_PORT_CNT) {
    return (bf_pltfm_set_cpu_module_lpmode(hndl, lpmode));
  }
...
int bf_pltfm_i2c_lock_select_misc_chn() {
  return (bf_mav_lock_select_misc_chn(qsfp_hndl[CP2112_ID_1]));
}
...
int bf_pltfm_unselect_i2c_unlock_misc_chn() {
  return (bf_mav_unselect_unlock_misc_chn(qsfp_hndl[CP2112_ID_1]));
}
