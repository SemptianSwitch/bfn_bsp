...
static bf_pltfm_cp2112_device_ctx_t *tfn_slv_i2c_hdl;

static int raw_read(bf_pltfm_cp2112_device_ctx_t *hndl,
                    uint8_t address,
                    uint8_t *offset,
                    int size_offset,
                    uint8_t *buf,
  ...
  ret = bf_pltfm_cp2112_write(hndl, address, offset, size_offset, 100);
  if (ret) {
    goto raw_read_error;
  }
  ret = bf_pltfm_cp2112_read(hndl, address, buf, len, 100);
...
static int raw_write(bf_pltfm_cp2112_device_ctx_t *hndl,
                     uint8_t address,
                     uint8_t *buf,
                     int len) {
  int ret;
  ...
  ret = bf_pltfm_cp2112_write(hndl, address, buf, len, 100);
...
static int bf_pltfm_i2c_init(int chip_id) {
  tfn_slv_i2c_hdl = bf_pltfm_cp2112_get_handle(CP2112_ID_1);
  if (!tfn_slv_i2c_hdl) {
...
int bf_pltfm_loc_i2c_rd(int chip_id, uint8_t local_reg, uint8_t *data) {
  bf_pltfm_cp2112_device_ctx_t *upper = tfn_slv_i2c_hdl;

  local_reg &= ~TOFINO_I2C_CMD_OPCODE_MASK;
  local_reg |= TOFINO_I2C_OPCODE_LOCAL_RD;
  return (raw_read(upper, get_i2c_addr(chip_id), &local_reg, 1, data, 1));
}
...
int bf_pltfm_loc_i2c_wr(int chip_id, uint8_t local_reg, uint8_t data) {
  ...
  out[0] = local_reg;
  out[1] = data;
  return (raw_write(upper, get_i2c_addr(chip_id), &out[0], 2));
}
...
int bf_pltfm_dir_i2c_rd(int chip_id,
  ...
  return (
      raw_read(upper, get_i2c_addr(chip_id), &out[0], 5, (uint8_t *)data, 4));
}

int bf_pltfm_switchd_dir_i2c_rd(int chip_id,
                                uint32_t dir_addr,
                                uint32_t *data) {
  return bf_pltfm_dir_i2c_rd(chip_id, TOFINO_I2c_BAR0, dir_addr, data);
}
...
int bf_pltfm_dir_i2c_wr(int chip_id,
  ...
  return (raw_write(upper, get_i2c_addr(chip_id), &out[0], 9));
}

int bf_pltfm_switchd_dir_i2c_wr(int chip_id, uint32_t dir_addr, uint32_t data) {
  return bf_pltfm_dir_i2c_wr(chip_id, TOFINO_I2c_BAR0, dir_addr, data);
}
...
int bf_pltfm_ind_i2c_rd(int chip_id, uint64_t indir_addr, uint8_t *data) {
  ...
  return (
      raw_read(upper, get_i2c_addr(chip_id), &out[0], 6, (uint8_t *)data, 16));
}
...
int bf_pltfm_ind_i2c_wr(int chip_id, uint64_t indir_addr, uint8_t *data) {
  ...
  return (raw_write(upper, get_i2c_addr(chip_id), &out[0], 22));
}
