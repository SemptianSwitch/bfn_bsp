...
static bf_pltfm_cp2112_device_ctx_t *si5342_i2c_hdl;
static int bf_pltfm_si5342_ppm = 0; /* cached value */
...
static si5342_register_op_t const si5342_322_322_cfg[] = {
...
static si5342_register_op_t const si5342_322_257_cfg[] = {
    /* Start configuration preamble */
    {0x0B24, 0xC0},
    {0x0B25, 0x00},
    {0x0540, 0x01},
...
static si5342_register_op_t const si5342_257_322_cfg[] = {
    /* Start configuration preamble */
    {0x0B24, 0xC0},
    {0x0B25, 0x00},
    {0x0540, 0x01},
...
static si5342_register_op_t const si5342_257_257_cfg[] = {
    /* Start configuration preamble */
    {0x0B24, 0xC0},
    {0x0B25, 0x00},
    {0x0540, 0x01},
...
static si5342_register_op_t const si5342_156_ppm_cfg[] = {
    /* Start configuration preamble */
    {0x0B24, 0xC0},
    {0x0B25, 0x00},
    {0x0540, 0x01},
...
int bf_pltfm_si5342_i2c_rd(int chip_id, uint32_t reg, uint8_t *data) {
  ...
  out[1] = (uint8_t)(reg >> 8);
  ret = bf_pltfm_cp2112_write(si5342_i2c_hdl, I2C_ADDR_SI5342, out, 2, 100);
  if (ret) {
  ...
  out[0] = (uint8_t)reg;
  ret = bf_pltfm_cp2112_write(si5342_i2c_hdl, I2C_ADDR_SI5342, out, 1, 100);
  if (ret) {
    LOG_ERROR("error writing register %x in si5342_i2c_rd\n", reg);
    goto si5342_read_error;
  }
  /* read the data into the destination register */
  ret = bf_pltfm_cp2112_read(si5342_i2c_hdl, I2C_ADDR_SI5342, data, 1, 100);
  if (ret) {
...
int bf_pltfm_si5342_ppm_set(int chip_id, int ppm) {
  ...
  if (bf_pltfm_si5342_ppm < ppm) { /* increment the ppm */
...
static ucli_status_t bf_pltfm_ucli_ucli__5342_i2c_rd(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  uint32_t reg;
  uint8_t data;

  UCLI_COMMAND_INFO(uc, "si5342-i2c-rd", 2, "si5342-i2c-rd <dev> 0x<reg>");
...
static ucli_status_t bf_pltfm_ucli_ucli__5342_i2c_wr(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  uint32_t reg;
  uint8_t data;

  UCLI_COMMAND_INFO(
      uc, "si5342-i2c-wr", 3, "si5342-i2c-wr <dev> 0x<reg> 0x<data>");
...
static ucli_status_t bf_pltfm_ucli_ucli__5342_ppm_set(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  int ppm;

  UCLI_COMMAND_INFO(uc, "si5342-ppm-set", 2, "si5342-ppm-set <dev> <ppm>");
...
static ucli_status_t bf_pltfm_ucli_ucli__5342_ppm_get(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  int ppm = 0;

  UCLI_COMMAND_INFO(uc, "si5342-ppm-get", 1, "si5342-ppm-get <dev>");

  dev = atoi(uc->pargs->args[0]);

  aim_printf(&uc->pvs, "bf_pltfm_si5342: si5342-ppm-get <dev=%d>\n", dev);
...
/** si5342 checkif device is ready
  ...
  *    0 on success and any other value on error
  */
static int bf_pltfm_si5342_dev_rdy(int chip_id, uint8_t *data) {
  uint8_t out[4];
  int ret;
...
static ucli_status_t bf_pltfm_ucli_ucli__5342_cfg_wr(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  uint32_t cfm_val;
  uint8_t data, i;

  UCLI_COMMAND_INFO(
      uc, "si5342-cfg-wr", 3, "si5342-cfg-wr <dev> 0x<cfmdata = 0x7AD1FB09>");
  ...
  aim_printf(&uc->pvs,
             ...
             dev,
             cfm_val);
  bf_sys_sleep(10); /* deliberate delay to allow ctrl-C */
  /* write 0xC7 to NVM_WRITE */
  if (bf_pltfm_si5342_i2c_wr(0, SI5342_NVM_WR_REG, 0xC7) != 0) {
    aim_printf(&uc->pvs, "error writing to NVM_WRITE register\n");
    return UCLI_STATUS_E_INTERNAL;
  }
  i = 0;
  ...
  while (i < 100) {
    if (bf_pltfm_si5342_dev_rdy(0, &data) != 0) {
      aim_printf(&uc->pvs, "error reading to DEV READY register\n");
      return UCLI_STATUS_E_INTERNAL;
    }
  ...
  aim_printf(&uc->pvs, "Timeout error in DEV READY register\n");
  return UCLI_STATUS_E_INTERNAL;
}
#endif
...
static int si_5342_cfg_write(ucli_context_t *uc,
  ...
  for (i = 0; i < cnt; i++) {
    if (i == PREMABLE_CFG_CNT) {
      bf_sys_usleep(300000); /* 300 ms delay after preamble */
    }
    if (bf_pltfm_si5342_i2c_wr(dev, cfg[i].address, cfg[i].value) != 0) {
      aim_printf(&uc->pvs,
...
int bf_pltfm_si5342_156mhz_ppm_config_set(int chip_id) {
  size_t size_cfg, i;
  si5342_register_op_t const *cfg = &si5342_156_ppm_cfg[0];

  size_cfg = sizeof(si5342_156_ppm_cfg) / sizeof(si5342_register_op_t);
  for (i = 0; i < size_cfg; i++) {
    if (i == PREMABLE_CFG_CNT) {
      bf_sys_usleep(300000); /* 300 ms delay after preamble */
    }
    if (bf_pltfm_si5342_i2c_wr(chip_id, cfg[i].address, cfg[i].value) != 0) {
...
static ucli_status_t bf_pltfm_ucli_ucli__5342_cfg_prog(ucli_context_t *uc) {
  int mode, dev;
  ...
  switch (mode) {
    case 0:
      ...
      size_cfg = sizeof(si5342_322_322_cfg) / sizeof(si5342_register_op_t);
      break;
    case 1:
      delta_cfg = &si5342_322_257_cfg[0];
      size_cfg = sizeof(si5342_322_257_cfg) / sizeof(si5342_register_op_t);
      break;
    case 2:
      delta_cfg = &si5342_257_322_cfg[0];
      size_cfg = sizeof(si5342_257_322_cfg) / sizeof(si5342_register_op_t);
      break;
    case 3:
      delta_cfg = &si5342_257_257_cfg[0];
      size_cfg = sizeof(si5342_257_257_cfg) / sizeof(si5342_register_op_t);
      break;
    case 4:
  ...
  aim_printf(&uc->pvs, "programming config for mode %d\n", mode);
  if (si_5342_cfg_write(uc, dev, delta_cfg, size_cfg) != 0) {
    return UCLI_STATUS_E_INTERNAL;
  }
  return UCLI_STATUS_OK;
}

static ucli_status_t bf_pltfm_ucli_ucli__5342_cfg_reset(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "si5342-cfg-reset", 1, "si5342-cfg-reset <dev>");
  si5342_base_cfg_done = false;
  return UCLI_STATUS_OK;
}

/* <auto.ucli.handlers.start> */
static ucli_command_handler_f bf_pltfm_si5342_ucli_ucli_handlers__[] = {
...
ucli_node_t *bf_pltfm_si5342_ucli_node_create(ucli_node_t *m) {
  ...
  n = ucli_node_create("si5342", m, &bf_pltfm_si5342_ucli_module__);
  ucli_node_subnode_add(n, ucli_module_log_node_create("si5342"));
  return n;
}

int bf_pltfm_si5342_init(void *arg) {
  (void)arg;
  si5342_i2c_hdl = bf_pltfm_cp2112_get_handle(CP2112_ID_1);
  if (!si5342_i2c_hdl) {
