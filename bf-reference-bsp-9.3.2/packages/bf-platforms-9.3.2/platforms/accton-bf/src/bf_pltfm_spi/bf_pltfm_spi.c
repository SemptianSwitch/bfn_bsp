...
int bf_pltfm_spi_wr(int chip_id, const char *fname, int offset, int size) {
  ...
  err = 0;
  if (bf_spi_eeprom_wr(chip_id, offset, buf, size) != BF_SUCCESS) {
...
int bf_pltfm_spi_rd(int chip_id, const char *fname, int offset, int size) {
  ...
  err = 0;
  if (bf_spi_eeprom_rd(chip_id, offset, buf, size) == BF_SUCCESS) {
...
static ucli_status_t bf_pltfm_ucli_ucli__spi_wr_(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  char fname[64];
  int offset;
  int size;
  ...
  UCLI_COMMAND_INFO(
      uc, "spi-wr", 4, "spi-wr <dev> <file name> <offset> <size>");

  dev = atoi(uc->pargs->args[0]);
  strncpy(fname, uc->pargs->args[1], sizeof(fname) - 1);
  ...
  if (bf_pltfm_spi_wr(dev, fname, offset, size)) {
    aim_printf(&uc->pvs, "error writing to SPI eeprom\n");
  } else {
...
static ucli_status_t bf_pltfm_ucli_ucli__spi_rd_(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  char fname[64];
  int offset;
  int size;
  ...
  UCLI_COMMAND_INFO(
      uc, "spi-rd", 4, "spi-rd <dev> <file name> <offset> <size>");
  ...
  if (bf_pltfm_spi_rd(dev, fname, offset, size)) {
    aim_printf(&uc->pvs, "error reading from SPI eeprom\n");
  } else {
...
static ucli_command_handler_f bf_pltfm_spi_ucli_ucli_handlers__[] = {
...
ucli_node_t *bf_pltfm_spi_ucli_node_create(ucli_node_t *m) {
  ...
  n = ucli_node_create("spi", m, &bf_pltfm_spi_ucli_module__);
  ucli_node_subnode_add(n, ucli_module_log_node_create("spi"));
  return n;
}
