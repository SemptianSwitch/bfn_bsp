 ...
 */
bf_pltfm_status_t rptr_reg_rd_wr(bf_pltfm_cp2112_device_ctx_t *dev,
  ...
  res = bf_pltfm_cp2112_write(dev, addr, write_buf, write_buf_size, TIMEOUT_MS);
  ...
  if (operation == RPTR_REG_READ) {
    ...
    res =
        bf_pltfm_cp2112_write(dev, addr, write_buf, write_buf_size, TIMEOUT_MS);
 ...
 * @return function call status
 */
bf_pltfm_status_t rptr_read(bf_pltfm_cp2112_device_ctx_t *dev,
                            bf_pltfm_rptr_i2c_addr_t *rptr_i2c_addr,
                            bf_pltfm_rptr_info_t *rptr_info) {
  uint8_t addr;
  uint8_t write_buf[2];
...
static ucli_status_t rptr_ucli_eq_show(bf_pltfm_port_info_t *port_info,
                                       bf_pltfm_rptr_mode_t mode) {
  bf_pltfm_rptr_info_t rptr_info;
  bf_pltfm_status_t res;

  res = bf_pltfm_bd_cfg_rptr_info_get(port_info, mode, 0, &rptr_info);
  ...
  res = bf_pltfm_rptr_operation(&rptr_info, mode, RPTR_READ, NULL, 0);
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_ig_eq_show__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "rptr_ig_eq_show", 2, "<conn_id> <channel_no>");
  ...
  return (rptr_ucli_eq_show(&port_info, BF_PLTFM_RPTR_INGRESS));
}

static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_eg_eq_show__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "rptr_eg_eq_show", 2, "<conn_id> <channel_no>");
  ...
  return (rptr_ucli_eq_show(&port_info, BF_PLTFM_RPTR_EGRESS));
}

static ucli_status_t rptr_ucli_eq_set(ucli_context_t *uc,
                                      uint8_t port_num,
                                      uint8_t channel_num,
                                      uint8_t eq_bst1,
                                      uint8_t eq_bst2,
  ...
  res = bf_pltfm_rptr_operation(&rptr_info, mode, RPTR_WRITE, NULL, 0);
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_ig_eq_set__(
    ucli_context_t *uc) {
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_eg_eq_set__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc,
                    "rptr_eg_eq_set",
                    9,
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_eg_eq_set_all__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc,
      "rptr_eg_eq_set_all",
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_ig_eq_set_all__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc,
      "rptr_ig_eq_set_all",
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_debug__(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc, "debug_mode", 1, "enable (1) or disable (0) debug <value>");
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_port_default_eq_set__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc,
                    "default_eq_set",
                    1,
  ...
  if (bf_pltfm_rptr_conn_eq_set(port_num) != BF_PLTFM_SUCCESS) {
    return UCLI_STATUS_E_ERROR;
  }
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_port_eq_show__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc, "port_eq_show", 1, "Show eq values for a port <port number>");
  ...
  for (i = 0; i < 4; i++) {
    port_info.chnl_id = i;
    printf("Channel %d:  ", i);
    rptr_ucli_eq_show(&port_info, BF_PLTFM_RPTR_INGRESS);
  }
  ...
  for (i = 0; i < 4; i++) {
    port_info.chnl_id = i;
    printf("Channel %d:  ", i);
    rptr_ucli_eq_show(&port_info, BF_PLTFM_RPTR_EGRESS);
  }

  return 0;
}

static ucli_status_t rptr_ucli_reg_rd_wr(bf_pltfm_port_info_t *port_info,
                                         bf_pltfm_rptr_mode_t mode,
                                         uint8_t *data,
                                         uint8_t operation,
                                         uint8_t reg_type) {
  ...
  res = bf_pltfm_rptr_operation(&rptr_info, mode, operation, data, reg_type);
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_port_reg_read__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc,
                    "rptr_reg_read",
                    5,
  ...
  port_num = atoi(uc->pargs->args[0]);
  channel_num = atoi(uc->pargs->args[1]);
  data[0] = (uint8_t)strtol(uc->pargs->args[3], NULL, 16);

  if (!strncmp(uc->pargs->args[2], "ig", 2)) {
    mode = BF_PLTFM_RPTR_INGRESS;
  } else if (!strncmp(uc->pargs->args[2], "eg", 2)) {
  ...
  } else if (!strncmp(uc->pargs->args[4], "c", 1)) {
  ...
  return (rptr_ucli_reg_rd_wr(&port_info, mode, data, RPTR_REG_READ, reg_type));
}

static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_port_reg_read_all__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "rptr_reg_read_all", 1, "<conn_id>");
  ...
  for (i = 0; i < 12; i++) {
    data[0] = i;
    rptr_ucli_reg_rd_wr(&port_info, mode, data, RPTR_REG_READ, reg_type);
  }
...
static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_port_reg_write__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc,
                    "rptr_reg_write",
                    6,
  ...
  channel_num = atoi(uc->pargs->args[1]);
  data[0] = (uint8_t)strtol(uc->pargs->args[3], NULL, 16);
  data[1] = (uint8_t)strtol(uc->pargs->args[4], NULL, 16);
  ...
  if (!strncmp(uc->pargs->args[5], "g", 1)) {
    reg_type = RPTR_GLOBAL_REG;
  } else if (!strncmp(uc->pargs->args[5], "c", 1)) {
    reg_type = RPTR_CHANNEL_REG;
  } else {
    printf("invalid register type passed: 'g' or 'c'\n");
    return 0;
  ...
  return (
      rptr_ucli_reg_rd_wr(&port_info, mode, data, RPTR_REG_WRITE, reg_type));
}

static ucli_status_t bf_pltfm_rptr_ucli_ucli__rptr_port_reg_write_all__(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc,
      "rptr_reg_write_all",
  ...
  data[0] = (uint8_t)strtol(uc->pargs->args[1], NULL, 16);
  data[1] = (uint8_t)strtol(uc->pargs->args[2], NULL, 16);

  if (!strncmp(uc->pargs->args[0], "ig", 2)) {
    mode = BF_PLTFM_RPTR_INGRESS;
  } else if (!strncmp(uc->pargs->args[0], "eg", 2)) {
    mode = BF_PLTFM_RPTR_EGRESS;
  } else {
    printf("Invalid mode passed \n");
    return 0;
  ...
  if (!strncmp(uc->pargs->args[3], "g", 1)) {
    reg_type = RPTR_GLOBAL_REG;
  } else if (!strncmp(uc->pargs->args[3], "c", 1)) {
    reg_type = RPTR_CHANNEL_REG;
  } else {
    printf("invalid register type passed: 'g' or 'c'\n");
    return 0;
...
static ucli_command_handler_f bf_pltfm_rptr_ucli_ucli_handlers__[] = {
...
ucli_node_t *bf_pltfm_rptr_ucli_node_create(ucli_node_t *m) {
  ...
  n = ucli_node_create("rptr", m, &bf_pltfm_rptr_ucli_module__);
  ucli_node_subnode_add(n, ucli_module_log_node_create("rptr"));
  return n;
}
