...
static bf_pltfm_rptr_i2c_addr_t rptr_map[2][MAX_RPTR] = {
    {
 ...
 */
static bf_pltfm_status_t rptr_config_init(bf_pltfm_cp2112_device_ctx_t *dev,
 ...
 * @return function call status
 */
static bf_pltfm_status_t rptr_eg_set(bf_pltfm_cp2112_device_ctx_t *dev,
                                     uint8_t dev_i2c_addr,
  ...
  write_buf[1] = 0x90 | ((rptr_info->high_gain << 5) & 0x20) |
 ...
 * @return function call status
 */
static bf_pltfm_status_t rptr_ig_set(bf_pltfm_cp2112_device_ctx_t *dev,
                                     uint8_t dev_i2c_addr,
                                     bf_pltfm_rptr_info_t *rptr_info) {
  uint8_t addr;
  uint8_t write_buf[2];
...
static bf_pltfm_status_t rptr_eg_mode_write(
  ...
  if (rptr_eg_set(cp2112_handle, rptr_i2c_addr->dev_i2c_addr, rptr_info) !=
      BF_PLTFM_SUCCESS) {
    return BF_PLTFM_COMM_FAILED;
  }
...
static bf_pltfm_status_t rptr_ig_mode_write(
  ...
  if (rptr_ig_set(cp2112_handle, rptr_i2c_addr->dev_i2c_addr, rptr_info) !=
      BF_PLTFM_SUCCESS) {
    return BF_PLTFM_COMM_FAILED;
  }
 ...
 */
bf_pltfm_status_t bf_pltfm_rptr_operation(bf_pltfm_rptr_info_t *rptr_info,
  ...
  if ((operation != RPTR_WRITE) && (operation != RPTR_READ) &&
      (operation != RPTR_REG_READ) && (operation != RPTR_REG_WRITE)) {
  ...
  res = bf_pltfm_cp2112_write(
      cp2112_handle, PCA9548_I2C_ADDR, &buf, sizeof(buf), TIMEOUT_MS);
  ...
  if (operation == RPTR_WRITE) {
    ...
    if (mode == BF_PLTFM_RPTR_EGRESS) {
      res = rptr_eg_mode_write(cp2112_handle, &rptr_i2c_addr, rptr_info);
      if (res != BF_PLTFM_SUCCESS) {
        LOG_ERROR("Egress mode writes to repeater failed\n");
        goto com_error;
      }
    } else {
      res = rptr_ig_mode_write(cp2112_handle, &rptr_i2c_addr, rptr_info);
      if (res != BF_PLTFM_SUCCESS) {
    ...
    /* Read EQ setting */
    res = rptr_read(cp2112_handle, &rptr_i2c_addr, rptr_info);
    if (res != BF_PLTFM_SUCCESS) {
    ...
    res = rptr_reg_rd_wr(
        cp2112_handle, &rptr_i2c_addr, rptr_info, data, operation, reg_type);
    if (res != BF_PLTFM_SUCCESS) {
...
bf_pltfm_status_t bf_pltfm_rptr_ch_eq_set(bf_pltfm_port_info_t *port_info,
  ...
  if ((mode != BF_PLTFM_RPTR_EGRESS) && (mode != BF_PLTFM_RPTR_INGRESS)) {
 ...
 * @return Status of the API call.
 */
bf_pltfm_status_t bf_pltfm_rptr_conn_ig_eq_set(uint32_t conn_id) {
  uint8_t i = 0;
  bf_pltfm_status_t res;
  ...
  for (; i < 4; i++) {
    ...
    res =
        bf_pltfm_rptr_ch_eq_set(&port_info, BF_PLTFM_RPTR_INGRESS, &rptr_info);
 ...
 * @return Status of the API call.
 */
bf_pltfm_status_t bf_pltfm_rptr_conn_eg_eq_set(uint32_t conn_id) {
  uint8_t i = 0;
  bf_pltfm_status_t res;
  bf_pltfm_port_info_t port_info;
  bf_pltfm_rptr_info_t rptr_info;
  ...
  for (; i < 4; i++) {
    ...
    res = bf_pltfm_rptr_ch_eq_set(&port_info, BF_PLTFM_RPTR_EGRESS, &rptr_info);
 ...
 * @return Status of the API call.
 */
bf_pltfm_status_t bf_pltfm_rptr_conn_eq_set(uint32_t conn_id) {
  bf_pltfm_status_t res;
...
bf_pltfm_status_t bf_pltfm_rptr_config_set(bf_dev_id_t dev_id,
                                           uint32_t conn,
  ...
  for (chnl = 0; chnl < QSFP_NUM_CHN; chnl++) {
    ...
    sts = bf_pltfm_bd_cfg_rptr_info_get(
        &port_info, BF_PLTFM_RPTR_EGRESS, qsfp_type, &eg_rptr_info);
    if (sts != BF_PLTFM_SUCCESS) {
      LOG_ERROR(
    ...
    sts = bf_pltfm_bd_cfg_rptr_info_get(
        &port_info, BF_PLTFM_RPTR_INGRESS, qsfp_type, &ig_rptr_info);
    if (sts != BF_PLTFM_SUCCESS) {
      LOG_ERROR(
