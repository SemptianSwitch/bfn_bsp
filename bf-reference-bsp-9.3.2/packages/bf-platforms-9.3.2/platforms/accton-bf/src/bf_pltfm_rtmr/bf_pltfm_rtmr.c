...
static bf_pltfm_rtmr_i2c_addr_t rtmr_map[][MAX_RTMR] = {
...
static const uint8_t bf_rtmr_pca9535_i2c_addr[BF_RTMR_PCA9535_MAX] = {
    (0x20 << 1), (0x21 << 1)};

/* return i2c address of a PCA9535 device */
static void bf_rtmr_mav_pca9535_i2c_addr_get(int pca9535, uint8_t *i2c_addr) {
 ...
 */
static bf_pltfm_status_t bf_rtmr_mav_pca9535_reg_set(
  ...
  bf_rtmr_mav_pca9535_i2c_addr_get(pca9535, &i2c_addr);
 ...
 */
static bf_pltfm_status_t bf_rtmr_mav_pca9535_reg_get(
    bf_pltfm_cp2112_device_ctx_t *hndl,
    int pca9535,
    uint8_t reg_addr,
    uint16_t *val) {
...
static bf_pltfm_status_t bf_pltfm_rtmr_raw_reg_read(
  ...
  res = bf_pltfm_cp2112_write_read_unsafe(cp2112_handle,
                                          rtmr_i2c_addr,
                                          wr_buf,
                                          (uint8_t *)rd_buf,
                                          2,
...
static bf_pltfm_status_t bf_pltfm_rtmr_tf_raw_reg_read(
  ...
  if (!tf_i2c_init) {
    ...
    res |= bf_i2c_set_clk(0, TOFINO_PIN_PAIR, TOFINO_I2C_PERIOD);
    res |= bf_i2c_set_submode(0, TOFINO_PIN_PAIR, BF_I2C_MODE_REG);
    tf_i2c_init = 1;
  }
  ...
  res |= bf_i2c_rd_reg_blocking(
      0, TOFINO_PIN_PAIR, (rtmr_i2c_addr >> 1), n_offset, 2, buf, 2);
...
bf_pltfm_status_t bf_pltfm_rtmr_reg_read(bf_pltfm_port_info_t *port_info,
  ...
  memcpy(&rtmr_i2c_info,
         &rtmr_map[0][BF_PLTFM_GET_RTMR_INDEX(port_info->conn_id)],
  ...
  cp2112_handle = bf_pltfm_cp2112_get_handle(rtmr_i2c_info.cp2112_num);
  ...
  res = bf_pltfm_cp2112_write(
      cp2112_handle, PCA9548_I2C_ADDR, &wr_buf, sizeof(wr_buf), TIMEOUT_MS);
  ...
  res = bf_pltfm_rtmr_raw_reg_read(
      rtmr_i2c_info.dev_i2c_addr, cp2112_handle, offset, rd_buf);
 ...
 * This routine does not set the i2x muxes on the path to the retimers.
 */
static bf_pltfm_status_t bf_pltfm_rtmr_raw_reg_write(
    uint8_t rtmr_i2c_addr,
    uint16_t offset,
    uint16_t data,
    bf_pltfm_cp2112_device_ctx_t *cp2112_handle) {
  ...
  res = bf_pltfm_cp2112_write(
      cp2112_handle, rtmr_i2c_addr, wr_buf, 4, TIMEOUT_MS);
...
static bf_pltfm_status_t bf_pltfm_rtmr_tf_raw_reg_write(uint8_t rtmr_i2c_addr,
                                                        uint16_t offset,
...
#if !RTMR_DO_CP2112_I2C
static bf_pltfm_status_t bf_pltfm_rtmr_tf_stream_wr(uint8_t rtmr_i2c_addr,
                                                    uint16_t offset,
                                                    uint8_t *buf,
                                                    uint32_t buf_size) {
  bf_pltfm_status_t res = BF_PLTFM_SUCCESS;
  ...
  res |= bf_i2c_set_submode(0, TOFINO_PIN_PAIR, BF_I2C_MODE_STATEOUT);

  res |= bf_write_stateout_buf(0, TOFINO_PIN_PAIR, 0, buf, buf_size);

  if (res |= bf_i2c_issue_stateout(
          0, TOFINO_PIN_PAIR, (rtmr_i2c_addr >> 1), n_offset, 2, 0, buf_size)) {
    res |= bf_i2c_reset(0, TOFINO_PIN_PAIR);
    res |= bf_i2c_set_submode(0, TOFINO_PIN_PAIR, BF_I2C_MODE_REG);
    return res;
  }
  ...
  while (wait_cnt++ < 50) {
    ...
    bf_sys_usleep(10);
    status = bf_i2c_get_completion_status(0, TOFINO_PIN_PAIR, &complete);
    if (status != BF_SUCCESS) {
      /* Restore GPIO to i2c register mode */
      bf_i2c_set_submode(0, TOFINO_PIN_PAIR, BF_I2C_MODE_REG);
...
bf_pltfm_status_t bf_pltfm_rtmr_reg_write(bf_pltfm_port_info_t *port_info,
  ...
  res = bf_pltfm_rtmr_raw_reg_write(
      rtmr_i2c_info.dev_i2c_addr, offset, data, cp2112_handle);

  if (offset == RTMR_FW_IND_CTRL_REG) {
    res = bf_pltfm_rtmr_raw_reg_read(
        rtmr_i2c_info.dev_i2c_addr, cp2112_handle, offset, &rd_buf);
    if ((data == RTMR_RD_CRC) && (rd_buf != RTMR_RD_CRC_CFM)) {
      LOG_DEBUG(
          ...
          rd_buf);
               ...
               ((rd_buf != RTMR_INIT_REG_CFM) && ((rd_buf & 0xf00) != 0xf00))) {
      LOG_ERROR(
...
static bf_pltfm_status_t bf_pltfm_rtmr_do_fw_dl(
  ...
  res = bf_pltfm_rtmr_raw_reg_write(
      rtmr_i2c_addr, FW2, BF_RTMR_START_FW_LOAD, cp2112_handle);
  ...
  res = bf_pltfm_rtmr_raw_reg_write(
      rtmr_i2c_addr, FW1, BF_RTMR_RESET_MCU, cp2112_handle);
  ...
  /* Clear MCU reset */
  res = bf_pltfm_rtmr_raw_reg_write(rtmr_i2c_addr, FW1, 0, cp2112_handle);
  ...
  res = bf_pltfm_rtmr_raw_reg_write(rtmr_i2c_addr, FW2, 0, cp2112_handle);
...
#else
    bf_pltfm_rtmr_tf_stream_wr(rtmr_i2c_addr, FW0, (uint8_t *)&sec_buf[1], 32);
  ...
  res =
      bf_pltfm_rtmr_raw_reg_write(rtmr_i2c_addr, offset, wr_buf, cp2112_handle);
...
bf_pltfm_status_t bf_pltfm_rtmr_fw_dl(bf_pltfm_port_info_t *port_info,
  ...
  res = bf_pltfm_rtmr_do_fw_dl(
      rtmr_i2c_info.dev_i2c_addr, cp2112_handle, buffer, buf_size);
...
static bf_pltfm_status_t bf_pltfm_rtmr_reset_mask(uint8_t pca9535_id,
                                                  uint16_t mask) {
  bf_pltfm_cp2112_device_ctx_t *cp2112_handle;
  uint16_t n_val;
  uint16_t val = 0;
  ...
  /* Get cp2112 device info */
  cp2112_handle = bf_pltfm_cp2112_get_handle(CP2112_ID_2);

  /* Get the current value */
  bf_rtmr_mav_pca9535_reg_get(
      cp2112_handle, pca9535_id, MAV_PCA9535_REG_OUT, &val);

  n_val = val & ~mask;
  /* Set output direction for LP mode and disable LP mode */
  bf_rtmr_mav_pca9535_reg_set(
      cp2112_handle, pca9535_id, MAV_PCA9535_REG_OUT, n_val);
  /* make these 9535 direction = output */
  bf_rtmr_mav_pca9535_reg_set(
      cp2112_handle, pca9535_id, MAV_PCA9535_REG_CFG, 0);
...
bf_pltfm_status_t bf_pltfm_rtmr_reset(bf_pltfm_port_info_t *port_info) {
  ...
  return bf_pltfm_rtmr_reset_mask(rtmr_i2c_info.pca9535_id, mask);
}
...
bf_pltfm_status_t bf_pltfm_rtmr_reset_all(void) {
  ...
  /* Set output direction for LP mode and disable LP mode */
  bf_rtmr_mav_pca9535_reg_set(cp2112_handle, 0, MAV_PCA9535_REG_OUT, 0);
  bf_rtmr_mav_pca9535_reg_set(cp2112_handle, 1, MAV_PCA9535_REG_OUT, 0);
  /* make these 9535 direction = output */
  bf_rtmr_mav_pca9535_reg_set(cp2112_handle, 0, MAV_PCA9535_REG_CFG, 0);
  bf_rtmr_mav_pca9535_reg_set(cp2112_handle, 1, MAV_PCA9535_REG_CFG, 0);

  bf_sys_usleep(10000);

  bf_rtmr_mav_pca9535_reg_set(cp2112_handle, 0, MAV_PCA9535_REG_OUT, 0xffff);
  bf_rtmr_mav_pca9535_reg_set(cp2112_handle, 1, MAV_PCA9535_REG_OUT, 0xffff);
 ...
 * @return Status of the API call.
 */
static bf_pltfm_status_t bf_pltfm_rtmr_fware_dl_ports(uint8_t start_port,
                                                      uint8_t end_port,
                                                      uint16_t crc,
                                                      bool recover) {
...
static bf_pltfm_status_t bf_pltfm_rtmr_check_crc(
  ...
  if (rtmr_crc != crc) {
    ...
    res = bf_pltfm_rtmr_fware_dl_ports(
        port_info->conn_id, port_info->conn_id, crc, true);
    res |= bf_pltfm_rtmr_crc_rd(port_info, &rtmr_crc);
    if (res) {
  ...
  if (an_mode) {
    ...
    if (is_x4) {
      ...
      res |= bf_pltfm_rtmr_reg_write(port_info, RTMR_FW_AN_MODE_REG, 1);
             ...
             (speed == BF_SPEED_100G) || (speed == BF_SPEED_40G_NB)) {
    mode = 2;
...
bf_pltfm_status_t bf_pltfm_ext_phy_init(void) {
  ...
  res |= bf_pltfm_rtmr_fware_dl_ports(
      MIN_MVR_RTMR_PORTS, MAX_MVR_RTMR_PORTS, crc, false);
