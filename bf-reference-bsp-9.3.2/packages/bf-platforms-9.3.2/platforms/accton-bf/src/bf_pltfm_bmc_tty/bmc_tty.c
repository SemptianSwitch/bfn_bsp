...
static int tty_open(void) {
  ...
  do {
    if ((tty_fd = open(TTY_DEVICE, O_RDWR | O_NOCTTY | O_NDELAY)) > -1) {
      ...
      attr.c_oflag = 0;
      attr.c_lflag = 0;
      attr.c_cc[VMIN] = (unsigned char)((MAXIMUM_TTY_STRING_LENGTH > 0xFF)
                                            ? 0xFF
...
static int tty_exec_buf(unsigned long udelay, const char *str) {
  ...
  ret = write(tty_fd, tty_buf, strlen(tty_buf) + 1);
  if (ret == -1) {
    return ret;
  }
  usleep(udelay);
  ret = read(tty_fd, tty_buf, MAXIMUM_TTY_BUFFER_LENGTH);
  if (ret == -1) {
    return ret;
  }
...
int bmc_send_command(char *cmd, unsigned long udelay) {
  int i, ret = 0;
...
int bmc_command_read_int(int *value, char *cmd, int base) {
  int len;
  int i;
  char *prev_str = NULL;
...
int bmc_i2c_readb(uint8_t bus, uint8_t devaddr, uint8_t addr) {
  ...
  snprintf(
      cmd, sizeof(cmd), "i2cget -f -y %d 0x%x 0x%02x\r\n", bus, devaddr, addr);
...
int bmc_i2c_readraw(
    uint8_t bus, uint8_t devaddr, uint8_t addr, char *data, int data_size) {
  int data_len, i = 0;
  char cmd[64] = {0};
  char *str = NULL;
  snprintf(cmd,
