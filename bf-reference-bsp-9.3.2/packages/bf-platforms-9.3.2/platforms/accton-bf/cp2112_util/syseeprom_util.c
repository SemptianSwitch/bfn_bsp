...
int read_sys_eeprom(unsigned char *eeprom_hdr, int offset, int size) {
  unsigned int len = 0;
  process_request(6, cmd1);
  //  process_request(8, cmd2);
  // loop 60 bytes at a time and read
  while (len < size) {
    snprintf(start_offset, 8, "0x%02x", len + offset);
...
int write_sys_eeprom(unsigned char *eeprom, int eeprom_len) { return 0; }

int main(int argc, char **argv) {
  static uint8_t eeprom[2048];
  bf_pltfm_status_t sts;
  sts = bf_pltfm_cp2112_util_init(BF_PLTFM_BD_ID_MONTARA_P0C);
  ...
  //
  // get TLV INFO
  tlvinfo_header_t *hdr = (tlvinfo_header_t *)eeprom;
  ...
  while (len < totalLen) {
    tlvinfo_tlv_t *t =
        (tlvinfo_tlv_t *)((uint8_t *)eeprom + len + sizeof(tlvinfo_header_t));
    if (t->type == ((argc > 1) ? atoi(argv[1]) : 0x22)) {
