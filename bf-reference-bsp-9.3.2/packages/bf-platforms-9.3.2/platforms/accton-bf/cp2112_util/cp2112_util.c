...
static int proc_read(int count, char *cmd[]) {
  ...
  byte_buf_size = (uint8_t)strtol(cmd[2], NULL, 16);
...
int proc_addr_read(int count, char *cmd[], bool is_unsafe, uint8_t *rb) {
  if (count < 4) {
    if (is_unsafe == false) {
      printf("Error: Insufficient arguments\nUsage: %s\n", usage[2]);
    } else {
  ...
  cp2112_id_ = (uint8_t)strtol(cmd[0], NULL, 16);
  i2c_addr = (uint8_t)strtol(cmd[1], NULL, 16);
  read_byte_buf_size = (uint8_t)strtol(cmd[2], NULL, 16);
  write_byte_buf_size = (uint8_t)strtol(cmd[3], NULL, 16);

  if (read_byte_buf_size > 256 || write_byte_buf_size > 60) {
  ...
  if (is_unsafe == true) {
    sts = bf_pltfm_cp2112_write_read_unsafe(cp2112_dev,
                                            i2c_addr,
                                            write_byte_buf,
                                            read_byte_buf,
                                            write_byte_buf_size,
...
static uint32_t proc_server_reg_read(uint32_t addr) {
  uint16_t ret_val;
  uint8_t i2c_addr;
  uint32_t byte_buf_size;
...
static int proc_server_reg_write(uint32_t addr, uint32_t val) {
  uint8_t i2c_addr;
  uint32_t byte_buf_size;
  uint8_t *byte_buf = &static_byte_buf[0];
  uint8_t cp2112_id_;
...
static int proc_server(int count, char *cmd[]) {
  ...
  while (true) {
    ...
    n_read = read_from_socket(
        reg_chnl, (uint8_t *)cmd, 4);  // read command length r/w
    if (dbg_print) {
    ...
    if (n_read > 0) {
      ...
      n_read = read_from_socket(
          reg_chnl, (uint8_t *)cmd, cmd_len);  // read command r/w
      if (n_read == cmd_len) {
        if (cmd[0] == 'r') {
          if (dbg_print) {
            printf("%s\n", cmd);
          ...
          uint16_t addr = (uint16_t)strtoul(
              (const char *)conv_buf, NULL, 16);  //*((uint16_t*)&cmd[4]);
          if (dbg_print) {
          ...
          write_to_socket(reg_chnl, (uint8_t *)rtn_data, 7);
        } else if (cmd[0] == 'w') {
          if (dbg_print) {
            printf("%s\n", cmd);
          }
...
int process_request(int count, char *cmd[]) {
  if (strcmp(cmd[1], "read") == 0) {
    ...
    return proc_addr_read(count, &cmd[2], false, NULL);
  } else if (strcmp(cmd[1], "addr-read-unsafe") == 0) {
    return proc_addr_read(count, &cmd[2], true, NULL);
...
int main(int argc, char *argv[]) {
  ...
  bf_pltfm_status_t sts;
  uint8_t board_id = (uint8_t)strtol(argv[1], NULL, 16);
  bf_pltfm_board_id_t bd_id = BF_PLTFM_BD_ID_MAVERICKS_P0B;
  if (board_id == 0) {
