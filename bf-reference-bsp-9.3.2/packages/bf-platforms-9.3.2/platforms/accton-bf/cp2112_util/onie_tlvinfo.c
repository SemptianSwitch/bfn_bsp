...
uint32_t *FAST_FUNC crc32_filltable(uint32_t *crc_table, int endian) {
  ...
  if (!crc_table) crc_table = xmalloc(256 * sizeof(uint32_t));
...
uint32_t FAST_FUNC crc32_block_endian1(uint32_t val,
                                       const void *buf,
                                       unsigned len,
                                       uint32_t *crc_table) {
...
uint32_t FAST_FUNC crc32_block_endian0(uint32_t val,
                                       const void *buf,
                                       unsigned len,
                                       uint32_t *crc_table) {
  const void *end = (uint8_t *)buf + len;
...
unsigned long crc32(unsigned long crc, const unsigned char *buf, unsigned len) {
  ...
  return crc32_block_endian0(crc ^ 0xffffffffL, buf, len, global_crc32_table) ^
         0xffffffffL;
}
...
static int set_bytes(char *buf, const char *string, int *converted_accum) {
  ...
  for (i = 0, p = (char *)string; (i < TLV_VALUE_MAX_LEN) && (*p != 0); i++) {
    ...
    if (*p != 0) {
      ...
      if (byte >= 256) {
        printf(
            "ERROR: The value specified is greater than 255: (%u) "
            "in string: %s\n",
            byte,
            string);
        return -1;
  ...
  if ((i == TLV_VALUE_MAX_LEN) && (*p != 0)) {
    printf(
        "ERROR: Trying to assign too many bytes "
        "(max: %d) in string: %s\n",
        TLV_VALUE_MAX_LEN,
        string);
    return -1;
 ...
 */
bool is_checksum_valid(u_int8_t *eeprom) {
...
int read_eeprom(u_int8_t *eeprom) {
  ...
  tlvinfo_header_t *eeprom_hdr = (tlvinfo_header_t *)eeprom;
  tlvinfo_tlv_t *eeprom_tlv =
      (tlvinfo_tlv_t *)&eeprom[sizeof(tlvinfo_header_t)];
 ...
 */
static int is_sys_eeprom_valid() { return hw_eeprom_valid; }
...
bool tlvinfo_delete_tlv(u_int8_t *eeprom, u_int8_t code) {
  ...
  if (tlvinfo_find_tlv(eeprom, code, &eeprom_index)) {
    ...
    memcpy(&eeprom[eeprom_index],
           &eeprom[eeprom_index + tlength],
...
bool tlvinfo_add_tlv(u_int8_t *eeprom, int tcode, char *strval) {
  ...
  switch (tcode) {
    ...
    case TLV_CODE_DEVICE_VERSION:
      ...
      if (value >= 256) {
        printf(
            "ERROR: Device version must be 255 or less. Value "
            "supplied: %u\n",
            value);
        return (FALSE);
      }
    ...
    case TLV_CODE_MAC_SIZE:
      ...
      if (value >= 65536) {
        printf(
            "ERROR: MAC Size must be 65535 or less. Value "
            "supplied: %u\n",
            value);
        return (FALSE);
      }
    ...
    case TLV_CODE_CRC_32:
      printf(
          "WARNING: The CRC TLV is set automatically and cannot be set "
          "manually.\n");
      return (FALSE);
    case TLV_CODE_VENDOR_EXT:
    default:
