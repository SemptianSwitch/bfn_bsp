...
extern void mav_diag_parse_port_list(char *port_string, bool *port_arr);

static ucli_status_t mavericks_diags_ucli_ucli__linespeed_init__(
    ucli_context_t *uc) {
  ...
  status =
      mav_diag_service_linespeed_init(dev, start_port, num_ports, (char *)type);
...
static ucli_status_t mavericks_diags_ucli_ucli__linespeed_run__(
    ucli_context_t *uc) {
  ...
  status = mav_diag_service_linespeed_run(time, pkt_size, num_pkt);
  if (status == BF_SUCCESS) {
...
static ucli_status_t mavericks_diags_ucli_ucli__linespeed_stop__(
    ucli_context_t *uc) {
  bf_status_t status = BF_SUCCESS;

  UCLI_COMMAND_INFO(uc, "linespeed-stop", 0, "linespeed-stop");

  aim_printf(&uc->pvs, "mavericks_diags:: Linespeed stop\n");
...
static ucli_status_t mavericks_diags_ucli_ucli__linespeed_end__(
    ucli_context_t *uc) {
  bf_status_t status = BF_SUCCESS;

  UCLI_COMMAND_INFO(uc, "linespeed-end", 0, "linespeed-end");

  aim_printf(&uc->pvs, "mavericks_diags:: Linespeed end\n");
...
static ucli_status_t mavericks_diags_ucli_ucli__linespeed_show__(
    ucli_context_t *uc) {
  char resp_str[MAV_DIAG_FILE_RESP_MAX_SIZE];

  UCLI_COMMAND_INFO(uc, "linespeed-show", 0, "linespeed-show");

  aim_printf(&uc->pvs, "mavericks_diags:: Linespeed show \n");

  resp_str[0] = '\0';
  mav_diag_service_linespeed_show(resp_str, MAV_DIAG_FILE_RESP_MAX_SIZE);
  aim_printf(&uc->pvs, "%s\n", resp_str);

  return 0;
}

static ucli_status_t mavericks_diags_ucli_ucli__port_status__(
    ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  ...
  mav_diag_service_port_status_show(
      dev, dev_port, resp_str, MAV_DIAG_FILE_RESP_MAX_SIZE);
  aim_printf(&uc->pvs, "%s\n", resp_str);

  return 0;
}

static ucli_status_t mavericks_diags_ucli_ucli__port_status_all__(
    ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  ...
  aim_printf(&uc->pvs, "mavericks_diags:: port status-all <dev=%d>\n", dev);
...
static ucli_status_t mavericks_diags_ucli_ucli__phy_info__(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  char resp_str[MAV_DIAG_FILE_RESP_MAX_SIZE];
  ...
  aim_printf(&uc->pvs, "mavericks_diags:: phy info <dev=%d> \n", dev);

  resp_str[0] = '\0';
  mav_diag_service_phy_info_show(dev, resp_str, MAV_DIAG_FILE_RESP_MAX_SIZE);
  aim_printf(&uc->pvs, "%s\n", resp_str);

  return 0;
}

static ucli_status_t mavericks_diags_ucli_ucli__port_map__(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  char resp_str[MAV_DIAG_FILE_RESP_MAX_SIZE];
  ...
  aim_printf(&uc->pvs, "mavericks_diags:: port map <dev=%d> \n", dev);

  resp_str[0] = '\0';
  mav_diag_service_show_pmap(dev, resp_str, MAV_DIAG_FILE_RESP_MAX_SIZE);
  aim_printf(&uc->pvs, "%s\n", resp_str);

  return 0;
}

static ucli_status_t mavericks_diags_ucli_ucli__temp__(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  char resp_str[MAV_DIAG_FILE_RESP_MAX_SIZE];
  ...
  aim_printf(&uc->pvs, "mavericks_diags:: temp <dev=%d> \n", dev);

  resp_str[0] = '\0';
  mav_diag_service_show_temp(dev, resp_str, MAV_DIAG_FILE_RESP_MAX_SIZE);
  aim_printf(&uc->pvs, "%s\n", resp_str);

  return 0;
}

static ucli_status_t mavericks_diags_ucli_ucli__eyescan__(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  bf_dev_port_t dev_port = 0;
  const char *mode;
  int ber = -1, lane = -1;
  ...
  UCLI_COMMAND_INFO(
      uc, "eyescan", 5, "eyescan <dev> <port-num> <ber> <mode> <lane>");
...
static ucli_status_t mavericks_diags_ucli_ucli__led_set__(ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  bf_dev_port_t dev_port = -1;
  bf_status_t status = BF_SUCCESS;
  ...
  status = mav_diag_service_led_set(dev, dev_port, (char *)color);
  if (status == BF_SUCCESS) {
...
static ucli_status_t mavericks_diags_ucli_ucli__led_set_all__(
    ucli_context_t *uc) {
  bf_dev_id_t dev = 0;
  bf_dev_port_t dev_port = -1;
  bf_status_t status = BF_SUCCESS;
...
static ucli_command_handler_f mavericks_diags_ucli_ucli_handlers__[] = {
...
ucli_node_t *mavericks_diags_ucli_node_create(void) {
  ucli_node_t *n;
  ucli_module_init(&mavericks_diags_ucli_module__);
  n = ucli_node_create("mavericks", NULL, &mavericks_diags_ucli_module__);
  ucli_node_subnode_add(n, ucli_module_log_node_create("mavericks"));
  return n;
}

void mav_diag_register_ucli_node() {
  ucli_node_t *ucli_node = mavericks_diags_ucli_node_create();
