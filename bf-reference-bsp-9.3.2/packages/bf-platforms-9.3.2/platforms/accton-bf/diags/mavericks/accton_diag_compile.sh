...
cd $WORKSPACE/bf-sde*
cd p4studio_build
./p4studio_build.py --use-profile profiles/diags_profile.yaml --bsp-path $WORKSPACE/bf-reference-bsp-$buildNumber --bf-diags-configure-options='--with-tofino P4PPFLAGS=-DDIAG_POWER_ENABLE' --bf-platforms-configure-options='--with-accton-diags' --tofino-architecture='tofino'
...
cd $SDE/pkgsrc/bf-platforms/platforms/accton-bf/diags/mavericks
