...
bf_status_t mav_diag_get_loopback_mode_from_str(
  ...
  } else if (!strncmp(type, "INTPI", strlen("INTPI"))) {
    *loop_mode = BF_DIAG_PORT_LPBK_PHY;
  } else if (!strncmp(type, "INTPO", strlen("INTPO"))) {
    *loop_mode = BF_DIAG_PORT_LPBK_PHY;
...
char *mav_diag_helper_get_speed_string(bf_port_speed_t speed,
                                       char *ret_buffer) {
  char *buffer;
...
/* Get the FEC string */
char *mav_diag_helper_get_fec_string(bf_fec_type_t fec, char *ret_buffer) {
  char *buffer;
...
char *mav_diag_helper_get_autoneg_string(bf_an_state_e an_state,
                                         char *ret_buffer) {
  char *buffer;
...
char *mav_diag_helper_get_qsfp_code_string(Ethernet_compliance qsfp_code,
                                           char *ret_buffer) {
  char *buffer;
...
char *mav_diag_helper_get_port_string(bf_pal_front_port_handle_t *port_info,
                                      char *ret_buffer,
                                      int max_len) {
  ret_buffer[0] = '\0';
...
int mav_diag_get_num_active_pipes(bf_dev_id_t dev_id) {
...
bf_status_t mav_diag_save_linespeed_init_cmd(
  ...
  MAV_DIAG_GET_LINESPEED_PARAMS.start_port = start_port;
  MAV_DIAG_GET_LINESPEED_PARAMS.num_ports = num_ports;
  MAV_DIAG_GET_LINESPEED_PARAMS.loop_mode = loop_mode;
...
/* Set linespeed */
bf_status_t mav_diag_service_linespeed_init(bf_dev_id_t dev_id,
                                            int start_port,
                                            int num_ports,
                                            char *type) {
  ...
  mav_diag_get_loopback_mode_from_str(type, &loop_mode);
  ...
  */
  if ((loop_mode != BF_DIAG_PORT_LPBK_EXT) &&
    ...
    for (front_port = start_port; front_port < (start_port + num_ports);
      ...
      if (front_port < BF_DIAG_MAX_PORTS) {
        bf_pm_port_fec_set(dev_id, &port_info, BF_FEC_TYP_NONE);
      }
    }
  }
  ...
  if (status == BF_SUCCESS) {
    ...
    mav_diag_save_linespeed_init_cmd(
        dev_id, sess_hdl, start_port, num_ports, loop_mode);
  }
  return status;
}

/* Get the mac counters */
bf_status_t mav_diag_linespeed_counter_get(int interval) {
  bf_status_t status = BF_SUCCESS;
  diag_front_port_t front_port = 0;
  bf_dev_port_t dev_port = 0;
  ...
  num_pipes = mav_diag_get_num_active_pipes(dev_id);
  for (front_port = MAV_DIAG_GET_LINESPEED_PARAMS.start_port;
       front_port < (MAV_DIAG_GET_LINESPEED_PARAMS.start_port +
    ...
    MAV_DIAG_GET_PORT_STATS(front_port, interval).rx_bytes =
        counters.format.ctr_ids.OctetsReceived;
    MAV_DIAG_GET_PORT_STATS(front_port, interval).rx_pkts =
        counters.format.ctr_ids.FramesReceivedAll;
    MAV_DIAG_GET_PORT_STATS(front_port, interval).rx_err_pkts =
        counters.format.ctr_ids.FrameswithanyError;
    MAV_DIAG_GET_PORT_STATS(front_port, interval).rx_fcs_err_pkts =
        counters.format.ctr_ids.FramesReceivedwithFCSError;
    MAV_DIAG_GET_PORT_STATS(front_port, interval).tx_pkts =
        counters.format.ctr_ids.FramesTransmittedAll;
    MAV_DIAG_GET_PORT_STATS(front_port, interval).tx_err_pkts =
...
bf_status_t mav_diag_service_linespeed_run(int time,
                                           int pkt_size,
                                           int num_packet) {
  bf_status_t status = BF_SUCCESS;
  bool bidir = false;
  ...
  status = bf_diag_loopback_pair_test_start(
      MAV_DIAG_GET_LINESPEED_PARAMS.sess_hdl, num_packet, pkt_size, bidir);

  if (status == BF_SUCCESS) {
    MAV_DIAG_GET_LINESPEED_PARAMS.time = time;
    MAV_DIAG_GET_LINESPEED_PARAMS.pkt_size = pkt_size;
    MAV_DIAG_GET_LINESPEED_PARAMS.num_packet = num_packet;
...
bf_status_t mav_diag_service_linespeed_end() {
  ...
  status = bf_diag_loopback_pair_test_cleanup(
      MAV_DIAG_GET_LINESPEED_PARAMS.sess_hdl);
  if (status == BF_SUCCESS) {
    if ((MAV_DIAG_GET_LINESPEED_PARAMS.loop_mode != BF_DIAG_PORT_LPBK_EXT) &&
        (MAV_DIAG_GET_LINESPEED_PARAMS.loop_mode != BF_DIAG_PORT_LPBK_NONE)) {
      ...
      for (front_port = MAV_DIAG_GET_LINESPEED_PARAMS.start_port;
           ...
           front_port++) {
        port_info.conn_id = front_port;
        fp_status = bf_pm_port_front_panel_port_to_dev_port_get(
            &port_info, &dev_id_of_port, &dev_port);
        ...
        if ((front_port < BF_DIAG_MAX_PORTS) &&
          ...
          bf_pm_port_fec_set(MAV_DIAG_GET_LINESPEED_PARAMS.dev_id,
                             &port_info,
...
bf_status_t mav_diag_service_linespeed_show(char *resp_str, int max_str_len) {
  ...
  test_status = bf_diag_loopback_pair_test_status_get(
      MAV_DIAG_GET_LINESPEED_PARAMS.sess_hdl, &cpu_stats);
  ...
  for (front_port = MAV_DIAG_GET_LINESPEED_PARAMS.start_port;
    ...
    /* Convert to strings */
    mav_diag_helper_get_speed_string(speed, speed_str);
    ...
    tx_err_pkts = 0;
#else
    rx_pkts = MAV_DIAG_GET_PORT_STATS(front_port, 1).rx_pkts -
              MAV_DIAG_GET_PORT_STATS(front_port, 0).rx_pkts;
    rx_err_pkts = MAV_DIAG_GET_PORT_STATS(front_port, 1).rx_err_pkts -
                  MAV_DIAG_GET_PORT_STATS(front_port, 0).rx_err_pkts;
    rx_fcs_err_pkts = MAV_DIAG_GET_PORT_STATS(front_port, 1).rx_fcs_err_pkts -
                      MAV_DIAG_GET_PORT_STATS(front_port, 0).rx_fcs_err_pkts;
    tx_pkts = MAV_DIAG_GET_PORT_STATS(front_port, 1).tx_pkts -
              MAV_DIAG_GET_PORT_STATS(front_port, 0).tx_pkts;
    tx_err_pkts = MAV_DIAG_GET_PORT_STATS(front_port, 1).tx_err_pkts -
                  MAV_DIAG_GET_PORT_STATS(front_port, 0).tx_err_pkts;
#endif

    /* Calculate packet speed */
    if (MAV_DIAG_GET_LINESPEED_PARAMS.time) {
      /* convert to mega bits/s */
      mbps = ((rx_pkts) * (MAV_DIAG_GET_LINESPEED_PARAMS.pkt_size + preamble) *
              8) /
...
bf_status_t mav_diag_service_port_status_show(
  ...
  for (front_port = 1; front_port <= num_ports; front_port++) {
    ...
    mav_diag_helper_get_autoneg_string(an_state, autoneg_str);
    /* Is port in loop */
    ...
    mav_diag_helper_get_qsfp_code_string(qsfp_code, qsfp_code_str);
    qsfp_ext_code = 0;
    bf_qsfp_get_eth_ext_compliance(front_port, &qsfp_ext_code);
    mav_diag_helper_get_qsfp_ext_code_string(qsfp_ext_code, qsfp_ext_code_str);
    ...
    tx_main = 0;
    tx_post = 0;
    bf_serdes_tx_drv_attn_get(dev_id, dev_port, 0, &tx_main, &tx_post, &tx_pre);
...
/* Show phy info */
bf_status_t mav_diag_service_phy_info_show(bf_dev_id_t dev_id,
                                           char *resp_str,
                                           int max_str_len) {
  int c_len = strlen(resp_str);
  int num_ports = 0;
...
/* Show port map */
bf_status_t mav_diag_service_show_pmap(bf_dev_id_t dev_id,
                                       char *resp_str,
                                       int max_str_len) {
  int c_len = strlen(resp_str);
  int num_ports = 0;
...
/* Show temperature */
bf_status_t mav_diag_service_show_temp(bf_dev_id_t dev_id,
                                       char *resp_str,
                                       int max_str_len) {
...
/* Create vlan */
bf_status_t mav_diag_service_vlan_create(bf_dev_id_t dev_id,
                                         int vlan_id,
                                         int pbm[],
                                         int num_pbm_ports,
                                         int ubm[],
...
/* Add ports to vlan */
bf_status_t mav_diag_service_vlan_add(bf_dev_id_t dev_id,
                                      int vlan_id,
                                      int pbm[],
                                      int num_pbm_ports,
                                      int ubm[],
  ...
  for (index = 0; index < num_ubm_ports; index++) {
    ...
    status =
        bf_pm_port_front_panel_port_to_dev_port_get(dev_id, &port_info, &port);
    if (status != BF_PLTFM_SUCCESS) {
      MAV_DIAG_PRINT(" Failed to get dev-port for front-port %d \n",
                     ubm[index]);
      continue;
...
/* Destroy vlan */
bf_status_t mav_diag_service_vlan_destroy(bf_dev_id_t dev_id, int vlan_id) {
  return bf_diag_vlan_destroy(dev_id, vlan_id);
}

/* Vlan show helper */
bf_status_t mav_diag_vlan_show_helper(bf_dev_id_t dev_id,
                                      int vlan_id,
                                      char *resp_str,
                                      int max_str_len,
                                      int *curr_len) {
...
/* Show vlan */
bf_status_t mav_diag_service_vlan_show(bf_dev_id_t dev_id,
                                       int input_vlan_id,
                                       char *resp_str,
                                       int max_str_len) {
  ...
  if (input_vlan_id != -1) {
    ...
    while (vlan_id != -1) {
      mav_diag_vlan_show_helper(dev_id, vlan_id, resp_str, max_str_len, &c_len);
...
/* Run eyescan command  */
bf_status_t mav_diag_service_eyescan(bf_dev_id_t dev_id,
                                     ...
                                     int max_str_len) {
  int c_len = strlen(resp_str);
  bf_sds_rx_eye_meas_mode_t meas_mode = BF_SDS_RX_EYE_MEAS_HEIGHT;
  ...
  if (mode && (!strncmp(mode, "line", strlen("line")))) {
  ...
  bf_serdes_rx_eye_3d_get(
      dev_id, dev_port, lane, meas_ber, resp_str, max_str_len - c_len - 1);

  return BF_SUCCESS;
}

/* Set LED color */
bf_status_t mav_diag_service_led_set(bf_dev_id_t dev_id,
                                     diag_front_port_t input_front_port,
                                     char *color) {
...
/* Setup snake */
bf_status_t mav_diag_service_snake_init(bf_dev_id_t dev_id,
                                        int start_port,
                                        int num_ports,
                                        char *type) {
  bf_diag_port_lpbk_mode_e loop_mode = BF_DIAG_PORT_LPBK_NONE;
  diag_front_port_t front_port = 0;
  bf_dev_port_t dev_port = 0, port_arr[MAV_DIAG_MAX_PORTS];
  int port_index = 0;
  bf_pal_front_port_handle_t port_info;
  bf_status_t status = BF_SUCCESS;
  bf_dev_id_t dev_id_of_port = 0;
...
bf_status_t mav_diag_service_snake_run(int pkt_size, int num_packet) {
  bool bidir = false;
...
static bf_status_t mav_diag_port_add_enable(bf_dev_id_t dev_id,
  ...
  sts = bf_pm_port_add(dev_id, &port_info, speed, fec);
  if (sts != BF_SUCCESS) {
...
#ifdef DEVICE_IS_SW_MODEL
static bf_status_t mav_diag_port_delete(bf_dev_id_t dev_id,
                                        bf_dev_port_t dev_port) {
  bf_status_t sts = BF_SUCCESS;
  bf_pal_front_port_handle_t port_info;
...
/* Ports add */
bf_status_t mav_diag_ports_add(bf_dev_id_t dev_id) {
  unsigned int pipe, port;
  uint32_t num_pipes;
  ...
  for (pipe = 0; pipe < num_pipes; pipe++) {
    for (port = 0; port < 64; port += 1) {
      mav_diag_port_delete(dev_id, MAKE_DEV_PORT(pipe, port));
    }
  }
#endif
  ...
  for (pipe = 0; pipe < num_pipes; pipe++) {
    for (port = 0; port < 64; port += 4) {
      mav_diag_port_add_enable(dev_id, MAKE_DEV_PORT(pipe, port), speed, fec);
    }
    ...
    if (pipe == 0) {
      ...
      if (has_mac) {
        mav_diag_port_add_enable(
            dev_id, MAKE_DEV_PORT(pipe, port), speed, BF_FEC_TYP_NONE);
      }
    }
#endif
  }
...
/* Set fec */
bf_status_t mav_diag_service_fec(bf_dev_id_t dev_id,
                                 int front_port,
                                 char *mode) {
  bf_status_t status = BF_SUCCESS;
  bf_fec_type_t fec;
...
/* Set port speed */
bf_status_t mav_diag_service_port_speed(bf_dev_id_t dev_id,
                                        int front_port,
                                        char *speed) {
  bf_status_t status = BF_SUCCESS;
  ...
  if (!strncmp(speed, "100000", strlen("100000"))) {
  ...
  status = bf_pm_port_speed_set(dev_id, &port_info, speed_val);

  return status;
}

/* Set port autoneg */
bf_status_t mav_diag_service_port_autoneg(bf_dev_id_t dev_id,
                                          int front_port,
                                          char *autoneg) {
  bf_status_t status = BF_SUCCESS;
...
/* Set port interface */
bf_status_t mav_diag_service_port_intf(bf_dev_id_t dev_id,
                                       int port,
                                       char *intf) {
  if (!intf) {
    return BF_INVALID_ARG;
...
/* Set pre-emphasis */
bf_status_t mav_diag_service_pre_emphasis(bf_dev_id_t dev_id,
                                          int front_port,
                                          int value,
                                          bool tx_pre,
                                          bool tx_post,
  ...
  for (chnl = 0; chnl < 4; chnl++) {
    ...
    if (tx_pre) {
      status |= bf_pm_port_serdes_tx_eq_pre_set(dev_id, &port_info, value);
    } else if (tx_post) {
      status |= bf_pm_port_serdes_tx_eq_post_set(dev_id, &port_info, value);
    } else if (tx_main) {
      status |= bf_pm_port_serdes_tx_eq_main_set(dev_id, &port_info, value);
    } else {
...
/* Prbs Set */
bf_status_t mav_diag_service_prbs_set(bf_dev_id_t dev_id,
                                      int front_port,
                                      uint32_t p_value,
  ...
  for (chnl = 0; chnl < 4; chnl++) {
    bf_qsfp_tx_disable_single_lane(front_port, chnl, false);
  ...
  if (status != BF_PLTFM_SUCCESS) {
    ...
    mav_diag_port_add_enable(dev_id, dev_port, speed, fec);
  }
...
/* Prbs Clean */
bf_status_t mav_diag_service_prbs_clean(bf_dev_id_t dev_id, int front_port) {
  bf_status_t status = BF_SUCCESS;
  bf_dev_port_t dev_port = 0;
  bf_port_speed_t speed = MAV_DIAG_DEF_PORT_SPEED;
  bf_fec_type_t fec = MAV_DIAG_DEF_PORT_FEC;
  ...
  status |= mav_diag_port_add_enable(dev_id, dev_port, speed, fec);

  return status;
}

/* Prbs Show */
bf_status_t mav_diag_service_prbs_show(bf_dev_id_t dev_id,
                                       int front_port,
                                       char *resp_str,
                                       int max_str_len) {
  int c_len = strlen(resp_str);
  ...
  for (chnl = 0; chnl < 4; chnl++) {
    ...
    bf_pm_port_prbs_stats_get(dev_id, &port_hdl, &stats.chnl_stats[chnl]);
  }
  for (chnl = 0; chnl < 4; chnl++) {
