...
bf_status_t mav_diag_create_server(int listen_port) {
...
static bf_status_t mav_diag_server_read_from_socket(int sock,
                                                    char *buf,
                                                    int len) {
...
/* Write to socket */
static void mav_diag_write_to_socket(int sock, char *buf, int len) {
...
static bf_status_t mav_diag_send_client_resp(uint8_t exit_code, char *dat) {
  char resp_str[MAV_DIAG_CLIENT_CMD_MAX_SIZE];
  int size = strlen(MAV_DIAG_CMD_OUT_FILENAME) + 1;

  mav_diag_info.resp_filep = fopen(MAV_DIAG_CMD_OUT_FILENAME, "w+");
  ...
  strncpy(resp_str + 1, MAV_DIAG_CMD_OUT_FILENAME, size);
  resp_str[size + 1] = 0; /* force null terminate */
  mav_diag_write_to_socket(mav_diag_info.client_sock, resp_str, size + 1);
...
bf_status_t mav_diag_parse_key_value_int(char *string) {
...
char *mav_diag_parse_key_value_string(char *string, char *buffer, int buf_len) {
  const char delimiters[] = "=";
  char *key, *value;
...
void mav_diag_parse_port_list(char *ports_bm, int *port_arr, int *num_ports) {
...
void mav_diag_parse_port_list_string(char *port_string,
                                     int *port_arr,
                                     int *num_ports) {
  ...
  mav_diag_parse_port_list(ports_bm, port_arr, num_ports);
}
...
static void mav_diag_process_linespeed_cmd(
    char subtokens[][MAV_DIAG_SUBTOKEN_STR_SIZE], int num_subtokens) {
  int ret = 0;
  if (num_subtokens < 1) {
  ...
  if (!strncmp(subtokens[0], "init", strlen("init"))) {
    ...
    /* Call the handler */
    ret = mav_diag_service_linespeed_run(time, pkt_size, num_packet);
    mav_diag_send_cmd_response(ret);
    ...
    /* Call the handler */
    ret = mav_diag_service_linespeed_show(mav_diag_info.file_resp_str,
                                          MAV_DIAG_FILE_RESP_MAX_SIZE);
    mav_diag_send_client_resp(ret, mav_diag_info.file_resp_str);
  } else {
...
static void mav_diag_process_ps_cmd(
  ...
  /* Call the handler */
  mav_diag_service_port_status_show(MAV_DIAG_DEF_DEV_ID,
                                    front_port,
                                    mav_diag_info.file_resp_str,
                                    MAV_DIAG_FILE_RESP_MAX_SIZE);
  mav_diag_send_client_resp(0, mav_diag_info.file_resp_str);
}
...
static void mav_diag_process_phy_cmd(
  ...
  if (!strncmp(subtokens[0], "info", strlen("info"))) {
    mav_diag_service_phy_info_show(MAV_DIAG_DEF_DEV_ID,
                                   mav_diag_info.file_resp_str,
                                   MAV_DIAG_FILE_RESP_MAX_SIZE);
    mav_diag_send_client_resp(0, mav_diag_info.file_resp_str);
    ...
    if (!strncmp(subtokens[2], "feyescan", strlen("feyescan"))) {
      for (i = 3; i < num_subtokens; i++) {
        if (!strncmp(subtokens[i], "u", strlen("u"))) {
          ber = mav_diag_parse_key_value_int(subtokens[i]);
        } else if (!strncmp(subtokens[i], "if", strlen("if"))) {
          mav_diag_parse_key_value_string(subtokens[i], mode, sizeof(mode));
        } else if (!strncmp(subtokens[i], "lane", strlen("lane"))) {
          lane = mav_diag_parse_key_value_int(subtokens[i]);
        }
      }
      /* Call handler */
      ...
      if (!strncmp(subtokens[3], "set", strlen("set"))) {
        ...
        for (cnt = 4; cnt <= 5; cnt++) {
          if (!strncmp(subtokens[cnt], "p", strlen("p"))) {
          ...
          } else if (!strncmp(subtokens[cnt], "speed", strlen("speed"))) {
        ...
        ret = mav_diag_service_prbs_set(
            MAV_DIAG_DEF_DEV_ID, atoi(port_name), p_value, speed_value);
        mav_diag_send_cmd_response(ret);
      } else if (!strncmp(subtokens[3], "clean", strlen("clean"))) {
        ret = mav_diag_service_prbs_clean(MAV_DIAG_DEF_DEV_ID, atoi(port_name));
        mav_diag_send_cmd_response(ret);
    ...
    bool tx_main = false, tx_post2 = false, tx_post3 = false;
    ...
    if (!strncmp(
      ...
      if (!strncmp(
              subtokens[2], "CL93N72_TXFIR_PRE", strlen("CL93N72_TXFIR_PRE"))) {
        valid_cmd = true;
        tx_pre = true;
      ...
      if (!strncmp(subtokens[2],
        ...
        tx_main = true;
                   ...
                   subtokens[1], "TXFIR_CTL1r.1", strlen("TXFIR_CTL1r.1"))) {
      if (!strncmp(subtokens[2], "TXFIR_POST2", strlen("TXFIR_POST2"))) {
        valid_cmd = true;
        tx_post2 = true;
      }
    } else if (!strncmp(
                   subtokens[1], "TXFIR_CTL2r.1", strlen("TXFIR_CTL2r.1"))) {
      if (!strncmp(subtokens[2], "TXFIR_POST3", strlen("TXFIR_POST3"))) {
        valid_cmd = true;
        tx_post3 = true;
      }
    }
    ...
    ret = mav_diag_service_pre_emphasis(MAV_DIAG_DEF_DEV_ID,
                                        atoi(port_name),
                                        emphasis_value,
                                        tx_pre,
                                        tx_post,
  ...
  if (!strncmp(subtokens[0], "create", strlen("create"))) {
    ...
    for (i = 2; i < num_subtokens; i++) {
      if (!strncmp(subtokens[i], "pbm", strlen("pbm"))) {
        mav_diag_parse_port_list_string(subtokens[i], &pbm[0], &num_pbm_ports);
      } else if (!strncmp(subtokens[i], "ubm", strlen("ubm"))) {
        mav_diag_parse_port_list_string(subtokens[i], &ubm[0], &num_ubm_ports);
      }
    }
    /* Call handler */
    ret = mav_diag_service_vlan_create(
        MAV_DIAG_DEF_DEV_ID, vlan_id, pbm, num_pbm_ports, ubm, num_ubm_ports);
    mav_diag_send_cmd_response(ret);
    ...
    vlan_id = atoi(subtokens[1]);
    /* Call handler */
    ret = mav_diag_service_vlan_destroy(MAV_DIAG_DEF_DEV_ID, vlan_id);
    mav_diag_send_cmd_response(ret);
  } else if (!strncmp(subtokens[0], "show", strlen("show"))) {
    if (num_subtokens >= 2) {
      vlan_id = atoi(subtokens[1]);
    ...
    /* Call handler */
    ret = mav_diag_service_vlan_show(MAV_DIAG_DEF_DEV_ID,
                                     vlan_id,
                                     mav_diag_info.file_resp_str,
                                     MAV_DIAG_FILE_RESP_MAX_SIZE);
    mav_diag_send_client_resp(0, mav_diag_info.file_resp_str);
...
static void mav_diag_process_led_cmd(
  ...
  if (led_port) {
    ...
    ret = mav_diag_service_led_set(MAV_DIAG_DEF_DEV_ID, front_port, def_color);
    mav_diag_send_cmd_response(ret);
    return;
  }
  ...
  ret = mav_diag_service_led_set(MAV_DIAG_DEF_DEV_ID, front_port, subtokens[0]);
...
static void mav_diag_process_snake_cmd(
  ...
  if (!strncmp(subtokens[0], "init", strlen("init"))) {
    ...
    /* Call the handler */
    ret = mav_diag_service_snake_run(pkt_size, num_packet);
    mav_diag_send_cmd_response(ret);
  } else if (!strncmp(subtokens[0], "stop", strlen("stop"))) {
    if (num_subtokens != 1) {
      mav_diag_process_invalid_cmd();
...
static void mav_diag_process_xport_cmd(
  ...
  if (!strncmp(subtokens[0], "fec", strlen("fec"))) {
    if (num_subtokens != 3) {
      mav_diag_process_invalid_cmd();
      return;
    }
    ...
    /* Call the handler */
    ret = mav_diag_service_fec(MAV_DIAG_DEF_DEV_ID, port, fec);
    mav_diag_send_cmd_response(ret);
  } else if (!strncmp(subtokens[0], "autoneg", strlen("autoneg"))) {
    if (num_subtokens != 3) {
      mav_diag_process_invalid_cmd();
      return;
    }
    ...
    /* Call the handler */
    ret = mav_diag_service_port_autoneg(MAV_DIAG_DEF_DEV_ID, port, autoneg);
    mav_diag_send_cmd_response(ret);
  } else {
    mav_diag_send_client_resp(1, "bad cmd param");
  }
...
static void mav_diag_process_port_cmd(
  ...
  if (!strncmp(subtokens[1], "speed", strlen("speed"))) {
    ...
    /* Call the handler */
    ret = mav_diag_service_port_speed(MAV_DIAG_DEF_DEV_ID, port, speed);
    mav_diag_send_cmd_response(ret);
    ...
    /* Call the handler */
    ret = mav_diag_service_port_intf(MAV_DIAG_DEF_DEV_ID, port, intf);
    mav_diag_send_cmd_response(ret);
  } else {
    mav_diag_send_client_resp(1, "bad cmd param");
  }
...
static void mav_diag_server_process_client_cmd(char *client_buf, int len) {
  const char delimiters[] = " ";
  char *cmd_token, cmd_str[MAV_DIAG_CLIENT_CMD_MAX_SIZE];
  char *subtoken,
  ...
  strncpy(cmd_str, client_buf, MAV_DIAG_CLIENT_CMD_MAX_SIZE);
  cmd_token = strtok(client_buf, delimiters);
  subtoken = strtok(NULL, delimiters);
  while (subtoken != NULL) {
    strncpy(subtokens[num_subtokens], subtoken, MAV_DIAG_SUBTOKEN_STR_SIZE);
    num_subtokens++;
    if (num_subtokens >= MAV_DIAG_SUBTOKEN_CNT_LIMIT) {
      break;
    }
    subtoken = strtok(NULL, delimiters);
  }
  if (!strncmp(cmd_token, "linespeed", strlen("linespeed"))) {
    mav_diag_process_linespeed_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "ps", strlen("ps"))) {
    mav_diag_process_ps_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "phy", strlen("phy"))) {
    mav_diag_process_phy_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "show", strlen("show"))) {
    mav_diag_process_show_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "vlan", strlen("vlan"))) {
    mav_diag_process_vlan_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "led_port", strlen("led_port"))) {
    mav_diag_process_led_cmd(subtokens, num_subtokens, true);
  } else if (!strncmp(cmd_token, "led", strlen("led"))) {
    mav_diag_process_led_cmd(subtokens, num_subtokens, false);
  } else if (!strncmp(cmd_token, "snake", strlen("snake"))) {
    mav_diag_process_snake_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "xport", strlen("xport"))) {
    mav_diag_process_xport_cmd(subtokens, num_subtokens);
  } else if (!strncmp(cmd_token, "port", strlen("port"))) {
    mav_diag_process_port_cmd(subtokens, num_subtokens);
...
bf_status_t mav_diag_server_init(void) {
  ...
  memset(&mav_diag_info, 0, sizeof(mav_diag_info));

  server_sock = mav_diag_create_server(MAV_DIAG_TCP_PORT_BASE_DEFAULT);
  ...
  while (1) {
    client_sock = get_mav_diags_client_conn(server_sock);
    ...
    rd_len = mav_diag_server_read_from_socket(client_sock, client_buf, cmd_len);
    mav_diag_server_process_client_cmd(client_buf, rd_len);
    bf_sys_usleep(10000);
  }
...
void *bf_switchd_agent_init(void *arg) {
  ...
  pthread_cleanup_push(mav_diag_exit_cleanup, "Mav Diag Cleanup handler");
