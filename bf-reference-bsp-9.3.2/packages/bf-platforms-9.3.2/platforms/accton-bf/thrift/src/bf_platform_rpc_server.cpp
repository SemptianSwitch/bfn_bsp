...
static void *bf_platform_rpc_server_thread(void *) {
  int port = BF_PLATFORM_RPC_SERVER_PORT;

  std::shared_ptr<pltfm_pm_rpcHandler> pltfm_pm_rpc_handler(
      new pltfm_pm_rpcHandler());

  std::shared_ptr<pltfm_mgr_rpcHandler> pltfm_mgr_rpc_handler(
  ...
  processor->registerProcessor(
      ...
      std::shared_ptr<TProcessor>(
          new pltfm_pm_rpcProcessor(pltfm_pm_rpc_handler)));
  processor->registerProcessor(
      "pltfm_mgr_rpc",
      std::shared_ptr<TProcessor>(
          new pltfm_mgr_rpcProcessor(pltfm_mgr_rpc_handler)));
...
int bf_platform_rpc_server_rmv(void *processor) {
  ...
  processor_->registerProcessor("pltfm_pm_rpc", std::shared_ptr<TProcessor>());
  processor_->registerProcessor("pltfm_mgr_rpc", std::shared_ptr<TProcessor>());
...
extern "C" {
int bf_platform_rpc_server_start(void **server_cookie) {
  pthread_mutex_init(&cookie_mutex, NULL);
  pthread_cond_init(&cookie_cv, NULL);

  std::cerr << "Starting BF-PLATFORM RPC server on port "
  ...
  *server_cookie = NULL;
  ...
  int status = pthread_create(
      &bf_pltfm_rpc_thread_id, NULL, bf_platform_rpc_server_thread, NULL);
...
int add_platform_to_rpc_server(void *processor) {
  std::cerr << "Adding Thrift service for bf-platforms to server\n";
...
int rmv_platform_from_rpc_server(void *processor) {
  std::cerr << "Removing Thrift service for bf-platforms from server\n";
