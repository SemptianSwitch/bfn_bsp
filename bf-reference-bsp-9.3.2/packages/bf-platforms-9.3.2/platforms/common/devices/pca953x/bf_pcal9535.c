...
static inline void pcal9535_lock(uint8_t device) {
  if (pca_info.device_lock) pca_info.device_lock(device);
}

static inline void pcal9535_unlock(uint8_t device) {
  if (pca_info.device_unlock) pca_info.device_unlock(device);
}

static bf_pltfm_status_t pcal9535_bf_read_port(uint8_t device,
                                               uint8_t dev_reg,
                                               uint16_t port_pos,
                                               uint8_t nbytes,
                                               uint16_t *port_val,
  ...
  ret = pca_info.read(device, dev_reg, nbytes, &port_map, user_data);
...
static bf_pltfm_status_t pcal9535_bf_write_port(uint8_t device,
                                                uint8_t dev_reg,
                                                uint16_t port_pos,
                                                bool set,
                                                uint8_t nbytes,
  ...
  if (port_pos != 0xffff) {
    ...
    ret |= pca_info.write(device, dev_reg, nbytes, &port_map, user_data);
  } else {
    if (set) {
      port_map = 0xffff;
    } else {
...
bf_pltfm_status_t pcal9535_bf_set_output_port(uint8_t device,
                                              uint8_t grp,
                                              uint16_t port_pos,
                                              bool enable,
                                              void *user_data) {
  ...
  ret = pcal9535_bf_write_port(
      device, dev_reg, port_pos, enable, nbytes, user_data);
  return (ret);
}

bf_pltfm_status_t pcal9535_bf_enable_input_latch(uint8_t device,
                                                 uint8_t grp,
                                                 uint16_t port_pos,
                                                 bool enable,
                                                 void *user_data) {
...
bf_pltfm_status_t pcal9535_bf_set_config_reg(uint8_t device,
                                             uint8_t grp,
                                             uint16_t port_pos,
                                             bool input,
                                             void *user_data) {
  ...
  ret = pcal9535_bf_write_port(
      device, dev_reg, port_pos, input, nbytes, user_data);
  return (ret);
}

bf_pltfm_status_t pcal9535_bf_set_pol_inv(uint8_t device,
                                          uint8_t grp,
                                          uint16_t port_pos,
                                          bool invert,
                                          void *user_data) {
  ...
  ret = pcal9535_bf_write_port(
      device, dev_reg, port_pos, invert, nbytes, user_data);
  return (ret);
}

bf_pltfm_status_t pcal9535_bf_get_output_port(uint8_t device,
                                              uint8_t grp,
                                              uint16_t port_pos,
                                              uint16_t *port_val,
                                              void *user_data) {
  ...
  ret = pcal9535_bf_read_port(
      device, dev_reg, port_pos, nbytes, port_val, user_data);
  return (ret);
}

bf_pltfm_status_t pcal9535_bf_get_config_reg(uint8_t device,
                                             uint8_t grp,
                                             uint16_t port_pos,
                                             uint16_t *port_val,
                                             void *user_data) {
...
bf_pltfm_status_t pcal9535_bf_get_pol_inv_reg(uint8_t device,
                                              uint8_t grp,
                                              uint16_t port_pos,
                                              uint16_t *port_val,
                                              void *user_data) {
...
bf_pltfm_status_t pcal9535_bf_get_input_port(uint8_t device,
                                             uint8_t grp,
                                             uint16_t port_pos,
                                             uint16_t *port_val,
                                             void *user_data) {
...
bf_pltfm_status_t pcal9535_bf_get_input_latch(uint8_t device,
                                              uint8_t grp,
                                              uint16_t port_pos,
                                              uint16_t *port_val,
                                              void *user_data) {
