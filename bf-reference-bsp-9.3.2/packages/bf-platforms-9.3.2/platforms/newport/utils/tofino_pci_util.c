...
static size_t tofino2_bar_size = (128 * 1024 * 1024);
static size_t tofino1_bar_size = (64 * 1024 * 1024);
static size_t tofino_bar_size = 0;

static void tofino_deinit(int tofino_id) {
  munmap(tofino_base_addr, tofino2_bar_size);
}
...
static int tofino_init(void *arg) {
  ...
  snprintf(tofino_name, sizeof(tofino_name), "%s%d", "/dev/bf", tofino_id);
  ...
  tofino_base_addr = mmap(
      NULL, tofino2_bar_size, PROT_READ | PROT_WRITE, MAP_SHARED, dev_fd, 0);
  if (tofino_base_addr == (uint8_t *)-1) {
    tofino_base_addr = mmap(
        NULL, tofino1_bar_size, PROT_READ | PROT_WRITE, MAP_SHARED, dev_fd, 0);
    if (tofino_base_addr) {
...
static char *usage[] = {
    "tofino_util reg_read <tofino_id> <reg_addr>"
    "tofino_util reg_write <tofino_id> <reg_addr> <32bit data>"};
...
static int proc_reg_read(int fd, int count, char *cmd[]) {
  ...
  val = *(volatile uint32_t *)(tofino_base_addr + reg);
...
static int proc_reg_write(int fd, int count, char *cmd[]) {
  ...
  *(volatile uint32_t *)(tofino_base_addr + reg) = val;
...
int main(int argc, char *argv[]) {
  ...
  uint8_t tofino_id = (uint8_t)strtol(argv[2], NULL, 0);
  ...
  int ret = process_request(tofino_id, argc - 1, &argv[1]);
