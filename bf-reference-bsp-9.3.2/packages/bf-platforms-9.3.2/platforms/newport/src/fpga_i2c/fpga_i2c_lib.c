...
#include <bf_fpga/bf_fpga_ioctl.h>
...
int bf_pltfm_fpga_init(void *arg) {
  ...
  snprintf(
      fpga_name, sizeof(fpga_name), "%s%d", BF_FPGA_DEV_BASE_NAME, fpga_id);
  dev_fd = open(fpga_name, O_RDWR);
  if (dev_fd < 0) {
    LOG_ERROR("Error in opening %s\n", fpga_name);
    return -1;
  }
  base_addr = mmap(
      NULL, BF_FPGA_BAR_SIZE(0), PROT_READ | PROT_WRITE, MAP_SHARED, dev_fd, 0);
  if (base_addr == (uint8_t *)-1) {
    LOG_ERROR("Error in mmapping %s\n", fpga_name);
    close(dev_fd);
    return -1;
  }
...
void bf_pltfm_fpga_deinit(void *arg) {
  ...
  if (fpga_id >= BF_FPGA_MAX_CNT) {
    LOG_ERROR("invalid fpga_id %d in closing the device\n", fpga_id);
    return;
  }
  if (bf_fpga_fd[fpga_id].dev_fd < 0 || !bf_fpga_fd[fpga_id].base_addr) {
    LOG_ERROR("Error in closing fpga_id %d\n", fpga_id);
    return;
  }
...
bf_pltfm_status_t bf_fpga_i2c_write(int fd,
                                    int bus,
                                    uint8_t delay,
                                    uint8_t addr,
                                    uint8_t *wr_buf,
...
bf_pltfm_status_t bf_fpga_i2c_write_byte(
    int fd, int bus, uint8_t delay, uint8_t addr, uint8_t val) {
  return bf_fpga_i2c_write(fd, bus, delay, addr, &val, 1);
}

bf_pltfm_status_t bf_fpga_i2c_addr_read(int fd,
                                        int bus,
                                        uint8_t delay,
                                        uint8_t addr,
                                        uint8_t *wr_buf,
...
bf_pltfm_status_t bf_fpga_i2c_read(int fd,
                                   int bus,
                                   uint8_t delay,
                                   uint8_t addr,
                                   uint8_t *rd_buf,
...
bf_pltfm_status_t bf_fpga_i2c_write_mux(int fd,
                                        int bus,
                                        uint8_t delay,
                                        uint8_t mux_addr,
                                        uint8_t mux_chn,
  ...
  i2c_op.i2c_inst[2].wr_buf[0] = 0x00; /* disable all mux channels */
...
bf_pltfm_status_t bf_fpga_i2c_read_mux(int fd,
                                       int bus,
                                       uint8_t delay,
                                       uint8_t mux_addr,
                                       uint8_t mux_chn,
...
bf_pltfm_status_t bf_fpga_i2c_addr_read_mux(int fd,
                                            int bus,
                                            uint8_t delay,
                                            uint8_t mux_addr,
                                            uint8_t mux_chn,
...
bf_pltfm_status_t bf_fpga_i2c_write_add_pr(int fd,
                                           int bus,
                                           uint8_t delay,
                                           uint8_t i2c_addr,
                                           uint8_t *wr_buf,
  ...
  populate_i2c_inst_ctrl_fields(
      &i2c_op, 0, false, true, i2c_addr, BF_FPGA_I2C_WRITE, delay, wr_sz, 0);
  memcpy(i2c_op.i2c_inst[1].wr_buf, wr_buf, wr_sz);
...
bf_pltfm_status_t bf_fpga_i2c_read_add_pr(int fd,
                                          int bus,
                                          uint8_t delay,
                                          uint8_t i2c_addr,
                                          uint8_t rd_sz,
  ...
  /* populate the i2c operation */
  populate_i2c_inst_ctrl_fields(
      &i2c_op, 0, false, true, i2c_addr, BF_FPGA_I2C_READ, delay, 0, rd_sz);
...
bf_pltfm_status_t bf_fpga_i2c_addr_read_add_pr(int fd,
                                               int bus,
                                               uint8_t delay,
                                               uint8_t i2c_addr,
                                               uint8_t *wr_buf,
  ...
  if (fd >= BF_FPGA_MAX_CNT || !wr_sz || !rd_sz || !wr_buf ||
      bus >= BF_I2C_FPGA_NUM_CTRL) {
    return BF_PLTFM_INVALID_ARG;
  }
  i2c_op.one_time = 0;
...
bf_pltfm_status_t bf_fpga_i2c_write_add_pr_mux(int fd,
                                               int bus,
                                               uint8_t delay,
  ...
  if (fd >= BF_FPGA_MAX_CNT || !wr_sz || !wr_buf ||
      bus >= BF_I2C_FPGA_NUM_CTRL || mux_i2c_addr > 0x7F) {
    return BF_PLTFM_INVALID_ARG;
  }
  i2c_op.one_time = 0;
  i2c_op.num_i2c = 3;
  ...
  /* populate first operation -> set mux channel */
  populate_i2c_inst_ctrl_fields(
      &i2c_op, 0, true, true, mux_i2c_addr, BF_FPGA_I2C_WRITE, delay, 1, 0);
  i2c_op.i2c_inst[0].wr_buf[0] = mux_chn;
  ...
  /* populate third operation -> reset mux channel */
  populate_i2c_inst_ctrl_fields(
      &i2c_op, 2, false, true, mux_i2c_addr, BF_FPGA_I2C_WRITE, 0, 1, 0);
  i2c_op.i2c_inst[2].wr_buf[0] = 0x00; /* disable all mux channels */
...
bf_pltfm_status_t bf_fpga_i2c_read_add_pr_mux(int fd,
                                              int bus,
                                              uint8_t delay,
                                              uint8_t mux_i2c_addr,
                                              uint8_t mux_chn,
...
bf_pltfm_status_t bf_fpga_i2c_addr_read_add_pr_mux(int fd,
                                                   int bus,
                                                   uint8_t delay,
                                                   uint8_t mux_i2c_addr,
                                                   uint8_t mux_chn,
...
static bf_pltfm_status_t bf_fpga_i2c_del_pr_internal(int fd,
                                                     int bus,
                                                     int inst_id,
                                                     uint8_t num_i2c) {
  bf_fpga_i2c_t i2c_op;
...
bf_pltfm_status_t bf_fpga_i2c_del_pr(int fd, int bus, int inst_id) {
  return bf_fpga_i2c_del_pr_internal(fd, bus, inst_id, 1);
}

bf_pltfm_status_t bf_fpga_i2c_del_pr_mux(int fd, int bus, int inst_id) {
  return bf_fpga_i2c_del_pr_internal(fd, bus, inst_id, 3);
}

bf_pltfm_status_t bf_fpga_i2c_read_data(int fd,
                                        int bus,
                                        int inst_id,
                                        uint8_t *rd_buf,
                                        uint8_t rd_sz,
...
/* clk in hz */
bf_pltfm_status_t bf_fpga_i2c_set_clk(int fd, int bus, int clk) {
...
bf_pltfm_status_t bf_fpga_i2c_start(int fd, int bus, bool start) {
...
bf_pltfm_status_t bf_fpga_i2c_is_running(int fd, int bus, bool *running) {
  bf_fpga_i2c_ctl_t i2c_ctl;
  int ret;
...
bf_pltfm_status_t bf_fpga_i2c_reset(int fd, int bus) {
  bf_fpga_i2c_ctl_t i2c_ctl;
  int ret;
...
bf_pltfm_status_t bf_fpga_i2c_en(int fd, int bus, int inst_id, bool en) {
  bf_fpga_i2c_ctl_t i2c_ctl;
  int ret;
