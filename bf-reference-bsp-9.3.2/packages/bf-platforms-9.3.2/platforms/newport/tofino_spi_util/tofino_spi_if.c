...
static int lld_spi_wr(
    int dev_id, uint8_t spi_code, uint8_t *w_buf, int w_size, int r_size) {
  uint32_t out_data0;
  ...
  if (w_size >= BF_SPI_MAX_OUTDATA || r_size > BF_SPI_MAX_INDATA) {
    return -2;
  }
  ...
  tofino_porting_spi_reg_wr(dev_id, offset, out_data0);
  offset = SPI_OUTDATA1_REG;
  tofino_porting_spi_reg_wr(dev_id, offset, out_data1);
  spi_cmd = bytes_to_wr | 0x80;
  spi_cmd |= (r_size << 4);
  /* printf("spi_wr out0 %08x out1 %08x cmd %08x\n", out_data0, out_data1,
   * spi_cmd); */
  offset = SPI_CMD_REG;
  tofino_porting_spi_reg_wr(dev_id, offset, spi_cmd);
  return 0;
}
 ...
 *
 */
static int lld_spi_get_idcode(int dev_id, uint32_t *spi_idcode) {
  uint32_t offset;

  offset = SPI_IDCODE_REG;
  return (tofino_porting_spi_reg_rd(dev_id, offset, spi_idcode));
}
#endif
 ...
 *
 */
static int lld_spi_get_rd_data(int dev_id, uint32_t *spi_rd_data) {
  uint32_t offset;

  offset = SPI_INDATA_REG;
  return (tofino_porting_spi_reg_rd(dev_id, offset, spi_rd_data));
}
 ...
 *
 */
static int lld_spi_eeprom_wr_enable(int dev_id, int en) {
  uint8_t spi_code = en ? BF_SPI_OPCODE_WR_EN : BF_SPI_OPCODE_WR_DI;
  /* w_size = r_size = 0 */
  return (lld_spi_wr(dev_id, spi_code, NULL, 0, 0));
}
...
static int bf_spi_eeprom_wr(int dev_id,
  ...
  if (buf_size < 0 || (addr + buf_size) > BF_SPI_EEPROM_SIZE) {
    return -2;
  }
  ...
  if (buf_size) {
    ...
    if (lld_spi_wr(
            dev_id, BF_SPI_OPCODE_WR_DATA, &wr_buf[0], buf_size + 2, 0) != 0) {
      return -1;
    }
    usleep(5000);
  }
 ...
 */
int bf_pltfm_spi_wr(int chip_id, FILE *fd, int offset, int size) {
  int err;
  uint8_t *buf;

  if (offset < 0 || (offset + size) > BF_SPI_EEPROM_SIZE) {
  ...
  err = 0;
  if (bf_spi_eeprom_wr(chip_id, offset, buf, size) != 0) {
 ...
 *    0 on success and -1 on error
 */
static int bf_pltfm_spi_rd(int chip_id, FILE *fd, int offset, int size) {
  int err;
  uint8_t *buf;
  ...
  err = 0;
  if (bf_spi_eeprom_rd(chip_id, offset, buf, size) == 0) {
...
static void print_usage(char *cmd) {
  ...
  TOFINO_PORTING_LOG_ERR(
      "%s <dev_id> <rd/wr> <custom-init-string 0: newport, 1: none>\n"
...
int main(int argc, char *argv[]) {
  ...
  if (tofino_porting_spi_init(dev_id, spi_init_str) != 0) {
