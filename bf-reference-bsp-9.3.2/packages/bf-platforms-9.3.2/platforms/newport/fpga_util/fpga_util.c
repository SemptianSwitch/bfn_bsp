...
static int proc_i2c_read(int fd, int count, char *cmd[]) {
  ...
  if (mux_i2c_addr >= 0x80) {
    ...
    ret = bf_fpga_i2c_read_mux(
        fd, bus, 0, mux_i2c_addr, mux_chn, i2c_addr, byte_buf, rd_size);
  }
...
static int proc_i2c_write(int fd, int count, char *cmd[]) {
  ...
  if (mux_i2c_addr >= 0x80) {
    ...
    ret = bf_fpga_i2c_write_mux(
        fd, bus, 0, mux_i2c_addr, mux_chn, i2c_addr, byte_buf, wr_size);
  }
...
static int proc_i2c_addr_read(int fd, int count, char *cmd[]) {
  ...
  if (mux_i2c_addr >= 0x80) {
    ret = bf_fpga_i2c_addr_read(
        fd, bus, 0, i2c_addr, byte_buf, byte_buf, wr_size, rd_size);
  } else {
    ret = bf_fpga_i2c_addr_read_mux(fd,
                                    bus,
                                    0,
                                    mux_i2c_addr,
                                    mux_chn,
  ...
  if (ret) {
    printf(
        "Error: addr read failed for bus %hhd addr 0x%02x \n", bus, i2c_addr);
    return -1;
  } else {
    for (i = 0; i < rd_size; i++) {
      printf("%02x ", (unsigned)byte_buf[i]);
...
/* stub functions to make fpga_util build */
