#!/bin/bash
# Start running Accton Diag compile

function print_help() {
  echo "USAGE: $(basename ""$0"") -s <bf-sde-path> -r <bf-reference-bsp-path>"
  echo "Options for running Accton Diag compile:"
  echo "  -s bf-sde-path"
...
cd $WORKSPACE/bf-sde*
cd p4studio_build
./p4studio_build.py --use-profile profiles/diags_profile.yaml --bsp-path $WORKSPACE/bf-reference-bsp-$buildNumber --bf-diags-configure-options='--with-tofino2 P4PPFLAGS=-DDIAG_POWER_ENABLE' --bf-platforms-configure-options='--with-newport-plat --with-accton-diags' --tofino-architecture='tofino2'

export SDE=$WORKSPACE/bf-sde-$buildNumber

echo "Compile completed successfully"

# building tar file for Accton
cd $SDE
mkdir -p bf_kdrv
...
cd $SDE/pkgsrc/bf-platforms/platforms/newport/accton_diags
...
echo "Accton diag image bf-diag-image.tar.gz is available in current directory"
