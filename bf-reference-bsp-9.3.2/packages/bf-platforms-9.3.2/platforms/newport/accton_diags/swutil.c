 ...
 *
 * Copyright (C) 2014  Accton Corporation
 */
#include <fcntl.h>
#include <errno.h>
#include <signal.h>
...
static int get_user_cmd_option(int argc, char *argv[]);
...
static void show_help(char *name) {
  ...
  for (i = 0; usage[i]; i++) printf(usage[i], name);
}
...
static int get_user_cmd_option(int argc, char *argv[]) {
  ...
  g_option = -1;
  opterr = 0;
  while ((opt = getopt(mgm_argc, mgm_argv, CMD_OPTION_LIST)) != -1) {
    ...
    switch (opt) {
      ...
      case 'S':
        ...
        if (optind >= mgm_argc) {
          ...
          strncpy(g_option_args, mgm_argv[optind], MAX_STR_LEN);
        }
        return 0;
...
static pid_t get_driver_process_id(void) {
  pid_t pid = 0;
  char buf[512];
  FILE *cmd_pipe = popen("pidof -s bf_switchd", "r");

  if (fgets(buf, 512, cmd_pipe) != NULL) {
    pid = strtoul(buf, NULL, 10);
  } else {
    printf("Failed to do get pid value of bf_switchd");
  }
...
static int do_op_command(int argc, char *argv[]) {
  ...
  char str[MAX_STR_LEN], cmd[MAX_STR_LEN], *cmd_ascii;
  ...
  for (i = 0; i < CFG_SWDRV_MAX_RETRY; i++) {
    t = connect(fd_socket, (struct sockaddr *)&server, sizeof(server));
    if (!t) break;
  ...
  snprintf(cmd_ascii, MAX_STR_LEN - sizeof(uint32_t), "%s", argv[1]);
  for (i = 2; i < argc; i++) {
    snprintf(cmd, MAX_STR_LEN, " %s", argv[i]);
    strncat(cmd_ascii, cmd, MAX_STR_LEN - strlen(cmd_ascii));
  }
  ...
  if (send(fd_socket, str, cmd_len + sizeof(uint32_t), 0) < 0) {
    perror("send");
    printf("failed to send command(%s) to switch driver!\n", str);
    return EXIT_FAILURE;
  }
  DB(printf("Command sent to Switch driver.\n"));
  if ((t = recv(fd_socket, str, MAX_STR_LEN, 0)) > 0) {
    str[t] = '\0';
    ret = str[0];
...
static int swdrv_start(char *options) {
  ...
  char exe_file[BUF_SIZE], conf_file[BUF_SIZE], install_path_arg[BUF_SIZE];
  int len = 0;
  pid_t pid = 0;
  char *parmList[5];
  int fd[2], new_fd = 0;
  ...
  snprintf(exe_file, BUF_SIZE, "%s/bin/bf_switchd", install_path);
  printf("exe_file is %s \n", exe_file);
  snprintf(install_path_arg, BUF_SIZE, "--install-dir=%s", install_path);
  printf("install_path_arg is %s \n", install_path_arg);
  snprintf(conf_file,
           BUF_SIZE,
           "--conf-file=%s/share/p4/targets/tofino2/accton_newport_diag.conf",
           install_path);
