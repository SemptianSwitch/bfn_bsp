...
static int port_num_is_valid(uint32_t conn, uint32_t chnl) {
  // Depending on the type of the board, check if the number of ports is valid
  if (conn < 1 || conn > (uint32_t)bf_bd_cfg_bd_num_port_get()) {
    return -1;
  }
...
static bf_pltfm_status_t str_to_port_info_map(const char *str,
  ...
  *ptr = '\0';
  ...
  if (port_num_is_valid(port_info->conn_id, port_info->chnl_id) != 0) {
    sts = BF_PLTFM_INVALID_ARG;
    goto err_cleanup;
  }
...
static int conn_and_chnl_get(const char *str, int *conn, int *chnl) {
  bf_pltfm_port_info_t port_info;
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_port_mac_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  uint32_t mac_id;
  ...
  UCLI_COMMAND_INFO(uc, "mac_get", 1, "<conn_id/chnl_id>");

  ret = conn_and_chnl_get(
      uc->pargs->args[0], (int *)&port_info.conn_id, (int *)&port_info.chnl_id);
  if (ret != 0) {
    aim_printf(&uc->pvs, "%s\n", usage);
    return 0;
  }
  ...
  bf_pltfm_status_t sts =
      bf_bd_cfg_port_mac_get(&port_info, &mac_id, &mac_chnl_id);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_port_tx_phy_lane_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  uint32_t tx_phy_lane_id;
  int ret;
  char usage[] = "Usage: tx_phy_lane_get <conn_id/chnl_id>";

  UCLI_COMMAND_INFO(uc, "tx_phy_lane_get", 1, "<conn_id/chnl_id>");
  ...
  bf_pltfm_status_t sts =
      bf_bd_cfg_port_tx_phy_lane_get(&port_info, &tx_phy_lane_id);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_port_rx_phy_lane_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  uint32_t rx_phy_lane_id;
  int ret;
  char usage[] = "Usage: rx_phy_lane_get <conn_id/chnl_id>";

  UCLI_COMMAND_INFO(uc, "rx_phy_lane_get", 1, "<conn_id/chnl_id>");
  ...
  bf_pltfm_status_t sts =
      bf_bd_cfg_port_rx_phy_lane_get(&port_info, &rx_phy_lane_id);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_port_tx_pn_swap_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  bool tx_pn_swap;
  int ret;
  char usage[] = "Usage: tx_pn_swap_get <conn_id/chnl_id>";

  UCLI_COMMAND_INFO(uc, "tx_pn_swap_get", 1, "<conn_id/chnl_id>");
  ...
  bf_pltfm_status_t sts =
      bf_bd_cfg_port_tx_pn_swap_get(&port_info, &tx_pn_swap);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_port_rx_pn_swap_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  bool rx_pn_swap;
  int ret;
  char usage[] = "Usage: rx_pn_swap_get <conn_id/chnl_id>";

  UCLI_COMMAND_INFO(uc, "rx_pn_swap_get", 1, "<conn_id/chnl_id>");
  ...
  bf_pltfm_status_t sts =
      bf_bd_cfg_port_rx_pn_swap_get(&port_info, &rx_pn_swap);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_port_mac_lane_info_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  ...
  qsfp_type = strtoul(uc->pargs->args[1], NULL, 10);
  ...
  bf_pltfm_status_t sts =
      bf_bd_cfg_port_mac_lane_info_get(&port_info, qsfp_type, &lane_info);
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_rptr_info_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  ...
  mode = (uint8_t)strtol(uc->pargs->args[1], NULL, 10);
  qsfp_type = strtoul(uc->pargs->args[2], NULL, 10);
  if (qsfp_type > BF_PLTFM_QSFP_OPT) {
    qsfp_type = BF_PLTFM_QSFP_CU_0_5_M;
  }
  ...
  bf_pltfm_status_t sts =
      bf_pltfm_bd_cfg_rptr_info_get(&port_info, mode, qsfp_type, &rptr_info);
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_rtmr_info_get__(
    ucli_context_t *uc) {
  bf_pltfm_port_info_t port_info;
  bf_pltfm_rtmr_info_t rtmr_info;
  int ret;
  char usage[] = "Usage: rtmr_info_get <conn_id/chnl_id>";
  bf_pltfm_status_t sts;
  bool rtmr_present;

  UCLI_COMMAND_INFO(uc, "rtmr_info_get", 1, "<conn_id/chnl_id>");
  ...
  sts = bf_bd_cfg_port_has_rtmr(&port_info, &rtmr_present);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(
        &uc->pvs,
  ...
  sts = bf_bd_cfg_rtmr_info_get(&port_info, &rtmr_info);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_bd_type_get__(
    ucli_context_t *uc) {
  char name[128];
...
static void print_detailed_info(ucli_context_t *uc,
                                bf_pltfm_port_info_t *port_info,
  ...
  sts = bf_bd_cfg_port_mac_get(port_info, &mac_id, &mac_chnl_id);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
  ...
  sts = bf_bd_cfg_port_has_rptr(port_info, &has_rptr);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
  ...
  sts = bf_pltfm_bd_cfg_rptr_info_get(
      port_info, BF_PLTFM_RPTR_EGRESS, qsfp_type, &e_rptr_info);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
               "ERROR: Unable to get the Repeater info for conn %d chnl %d\n",
               port_info->conn_id,
  ...
  sts = bf_pltfm_bd_cfg_rptr_info_get(
      port_info, BF_PLTFM_RPTR_INGRESS, qsfp_type, &i_rptr_info);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
               "ERROR: Unable to get the Repeater info for conn %d chnl %d\n",
               port_info->conn_id,
  ...
  sts = bf_bd_cfg_rptr_addr_get(
      port_info, BF_PLTFM_RPTR_EGRESS, &egress_i2c_addr);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
               "ERROR: Unable to get the Repeater info for conn %d chnl %d\n",
               port_info->conn_id,
  ...
  sts = bf_bd_cfg_rptr_addr_get(
      port_info, BF_PLTFM_RPTR_INGRESS, &ingress_i2c_addr);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
               "ERROR: Unable to get the Repeater info for conn %d chnl %d\n",
               port_info->conn_id,
  ...
  aim_printf(&uc->pvs, "Egress Repeater Number : %d\n", e_rptr_info.rptr_num);
  aim_printf(&uc->pvs, "Egress Repeater I2C Address : %d\n", egress_i2c_addr);
  aim_printf(&uc->pvs, "Egress Repeater Channel : %d\n", e_rptr_info.chnl_num);
  aim_printf(
      &uc->pvs, "Egress Repeater EQ Boost 1 : %d\n", e_rptr_info.eq_bst1);
  aim_printf(
      &uc->pvs, "Egress Repeater EQ Boost 2 : %d\n", e_rptr_info.eq_bst2);
  aim_printf(&uc->pvs, "Egress Repeater EQ BW : %d\n", e_rptr_info.eq_bw);
  aim_printf(&uc->pvs,
             "Egress Repeater EQ Bypass Boost 1 : %d\n",
             e_rptr_info.eq_bypass_bst1);
  aim_printf(&uc->pvs, "Egress Repeater VOD : %d\n", e_rptr_info.vod);
  aim_printf(&uc->pvs, "Egress Repeater RegF : %d\n", e_rptr_info.regF);
  aim_printf(
      &uc->pvs, "Egress Repeater High Gain : %d\n", e_rptr_info.high_gain);
  aim_printf(&uc->pvs, "Ingress Repeater Number : %d\n", i_rptr_info.rptr_num);
  aim_printf(&uc->pvs, "Ingress Repeater I2C Address : %d\n", ingress_i2c_addr);
  aim_printf(&uc->pvs, "Ingress Repeater Channel : %d\n", i_rptr_info.chnl_num);
  aim_printf(
      &uc->pvs, "Ingress Repeater EQ Boost 1 : %d\n", i_rptr_info.eq_bst1);
  aim_printf(
      &uc->pvs, "Ingress Repeater EQ Boost 2 : %d\n", i_rptr_info.eq_bst2);
  aim_printf(&uc->pvs, "Ingress Repeater EQ BW : %d\n", i_rptr_info.eq_bw);
  aim_printf(&uc->pvs,
             "Ingress Repeater EQ Bypass Boost 1 : %d\n",
             i_rptr_info.eq_bypass_bst1);
  aim_printf(&uc->pvs, "Ingress Repeater VOD : %d\n", i_rptr_info.vod);
  aim_printf(&uc->pvs, "Ingress Repeater RegF : %d\n", i_rptr_info.regF);
  aim_printf(
      &uc->pvs, "Ingress Repeater High Gain : %d\n", i_rptr_info.high_gain);
}

static void print_repeater_lane_info(ucli_context_t *uc,
                                     bf_pltfm_port_info_t *port_info,
                                     bf_pltfm_qsfp_type_t qsfp_type) {
  bf_pltfm_rptr_info_t e_rptr_info, i_rptr_info;
  bf_pltfm_status_t sts;
  ...
  aim_printf(&uc->pvs,
             "%2" PRIu64 "/%1" PRIu64 "|%2" PRIu64 "| %2" PRIx64 "| %2" PRIu64
             "| %2" PRIu64 " | %2" PRIu64 " |%2" PRIu64 "| %2" PRIu64
             " | %2" PRIu64 "|%2" PRIu64 "| %2" PRIx64 "| %2" PRIu64
             "| %2" PRIu64 " | %2" PRIu64 " |%2" PRIu64 "| %2" PRIu64
             " | %2" PRIu64 "\n",
...
static void print_lane_info_summary(ucli_context_t *uc,
                                    bf_pltfm_port_info_t *port_info,
                                    bf_pltfm_qsfp_type_t qsfp_type) {
  uint32_t mac_id;
  uint32_t mac_chnl_id;
  ...
  aim_printf(&uc->pvs,
             "%2" PRIu64 "/%1" PRIu64 "| %2" PRIu64 " |  %1" PRIu64
             " |  %2" PRIu64 " |   %1" PRIu64 " |  %2" PRIu64 " |   %1" PRIu64
             " |  %2" PRIu64 "  |  %2" PRIu64 "  |   %2" PRIu64 "  |  %1" PRIu64
             " \n",
             (uint64_t)port_info->conn_id,
             (uint64_t)port_info->chnl_id,
             (uint64_t)mac_id,
...
static void dump_info(ucli_context_t *uc,
  ...
  if (dflag) {
    aim_printf(
        ...
        "======================================================================"
        "==========\n");
    for (port_id = port_begin; port_id <= port_end; port_id++) {
      port_info.conn_id = port_id;
      for (lane_id = lane_begin; lane_id <= lane_end; lane_id++) {
  ...
  for (port_id = port_begin; port_id <= port_end; port_id++) {
    ...
    for (lane_id = lane_begin; lane_id <= lane_end; lane_id++) {
      port_info.chnl_id = lane_id;
      if (rflag) {
        print_repeater_lane_info(uc, &port_info, qsfp_type);
      } else {
        print_lane_info_summary(uc, &port_info, qsfp_type);
      }
    }
  }
}

static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__bd_cfg_dump__(
    ucli_context_t *uc) {
  extern char *optarg;
  extern int optind;
  ...
  while ((c = getopt(argc, argv, "p:q:dr")) != -1) {
    switch (c) {
      case 'p':
        pflag = 1;
        p_arg = optarg;
  ...
  if (pflag == 1) {
    ret = conn_and_chnl_get(
        p_arg, (int *)&port_info.conn_id, (int *)&port_info.chnl_id);
    if (ret != 0) {
      aim_printf(&uc->pvs, "%s\n", usage);
      return 0;
    }
    ...
    if (strchr(p_arg, '/')[1] >= '0' && strchr(p_arg, '/')[1] <= '3') {
      lflag = 1;
    }

    if (port_info.conn_id < 1 ||
        port_info.conn_id > (uint32_t)bf_bd_cfg_bd_num_port_get()) {
      aim_printf(&uc->pvs, "show: Invalid port number %d\n", port_info.conn_id);
      return 0;
    }
  ...
  dump_info(uc, num_port, num_lane, dflag, rflag, qsfp_type);
  return 0;
}


static ucli_status_t bf_pltfm_bd_cfg_ucli_ucli__tx_sds_eq_set__(
    ucli_context_t *uc) {
  ...
  uint32_t i, j;
  bf_pltfm_port_info_t port_info;
  static char usage[] = "Usage: tx_sds_eq_set <conn_id/chnl_id> <tx_attn> <tx_pre> <tx_post>";

  UCLI_COMMAND_INFO(uc, "tx_sds_eq_set", 4, "<conn_id/chnl_id> <tx_attn> <tx_pre> <tx_post>");

  sts = pltfm_port_str_to_info(uc->pargs->args[0], &port_info);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs, "%s\n", usage);
    return 0;
  }
  ...
  serdes_info.tx_attn = strtol(uc->pargs->args[1], NULL, 10);
  serdes_info.tx_pre = strtol(uc->pargs->args[2], NULL, 10);
  serdes_info.tx_post = strtol(uc->pargs->args[3], NULL, 10);
  ...
  for (i = conn_begin; i <= conn_end; i++) {
    for (j = chnl_begin; j <= chnl_end; j++) {
      ...
      sts = bf_bd_cfg_serdes_info_set(&port_info, &serdes_info);
      if (sts != BF_PLTFM_SUCCESS) {
        aim_printf(
            &uc->pvs,
...
static ucli_command_handler_f bf_pltfm_bd_cfg_ucli_ucli_handlers__[] = {
...
ucli_node_t *bf_bd_cfg_ucli_node_create(ucli_node_t *m) {
  ...
  n = ucli_node_create("bd_cfg", m, &bf_pltfm_bd_cfg_ucli_module__);
  ucli_node_subnode_add(n, ucli_module_log_node_create("bd_cfg"));
  return n;
}
