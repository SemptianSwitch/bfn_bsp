...
static cmis_app_host_id_map_t cmis_app_host_if_speed_map[] = {
    ...
    {29, BF_SPEED_400G, "400G             8 x 26.5625Gbd PAM4"},
};
...
static cmis_app_media_id_map_t cmis_app_media_if_speed_map[] = {
    ...
    {MEDIA_TYPE_SMF, 30, "400G-LR4         4 X 53.125 Gbd PAM4"},
...
static int bf_qsfp_bitfield_rmw_onebank(int port,
  ...
  uint8_t buf;  // create buffer for read data
  // only read & modify if we are partially updating the field
  if (chmask != 0xFF) {
    // read
    if (bf_qsfp_field_read_onebank(port, field, chmask, 0, 1, &buf) < 0) {
      return -1;
    }
  ...
  // write
  if (bf_qsfp_field_write_onebank(port, field, chmask, 0, 1, &buf) < 0) {
    return -1;
  }
  return 0;
}
...
int bf_qsfp_bitfield_rmw(int port,
  ...
  if ((port > bf_plt_max_qsfp) ||
      (chmask >= (1 << bf_qsfp_info_arr[port].num_ch))) {
    return -1;
  }

  /* loop through mask, 8 channels at a time since a CMIS bank is 8 channels */
  /* This routine also works for SFF-8636 */
  while (chmask >> (current_bank * 8)) {
    if (chmask & (0xFF << current_bank)) {  // only update banks in the mask

      if (bf_qsfp_bitfield_rmw_onebank(
              port,
              field,
              (chmask & (0xFF << current_bank)),
...
static int convert_flag_info_to_array_ind(int port,
  ...
  *bit_ind = flag_info->firstbit + (((ln_num * flag_info->laneincr) & 0x7) *
...
bool bf_qsfp_get_chan_sensor_data(int port, qsfp_channel_t *chn) {
  ...
  if (bf_qsfp_info_arr[port].num_ch == 4) {
    ...
    LOG_ERROR("bf_qsfp_get_chan_sensor_data: not supported channel count");
  }

  // outer loop handles banks, if applicable
  while (channel_base < bf_qsfp_info_arr[port].num_ch) {
    // read RX pwr flags
    if (bf_qsfp_field_read_onebank(
    ...
    for (channel = channel_base;
         ((channel < bf_qsfp_info_arr[port].num_ch) &&
          ((channel - channel_base) < MAX_CHAN_PER_CONNECTOR));
         channel++) {
      extract_qsfp_flags(CHANNEL_RX_PWR_ALARMS,
                         port,
                         data,
    ...
    for (channel = channel_base; ((channel < bf_qsfp_info_arr[port].num_ch) &&
                                  ((channel - channel_base) < 8));
         channel++) {
      extract_qsfp_flags(CHANNEL_TX_PWR_ALARMS,
                         port,
                         data,
                         channel - channel_base,
                         &chn[channel].sensors.tx_pwr.flags);
      chn[channel].sensors.tx_pwr._isset.flags = true;
    }
    ...
    for (channel = channel_base;
         ((channel < bf_qsfp_info_arr[port].num_ch) &&
          ((channel - channel_base) < MAX_CHAN_PER_CONNECTOR));
         channel++) {
      extract_qsfp_flags(CHANNEL_TX_BIAS_ALARMS,
                         port,
                         data,
                         channel - channel_base,
    ...
    for (channel = channel_base; ((channel < bf_qsfp_info_arr[port].num_ch) &&
                                  ((channel - channel_base) < 8));
         channel++) {
      if (channel < MAX_CHAN_PER_CONNECTOR) {
        val = data[channel * 2] << 8 | data[(channel * 2) + 1];
        chn[channel].sensors.rx_pwr.value = get_pwr(val);
        chn[channel].sensors.rx_pwr._isset.value = true;
      }
    }

    // read Tx pwr monitor values
    ...
    for (channel = channel_base; ((channel < bf_qsfp_info_arr[port].num_ch) &&
         ...
         channel++) {
      val = data[channel * 2] << 8 | data[(channel * 2) + 1];
      chn[channel].sensors.tx_bias.value = get_txbias(val);
...
int bf_qsfp_init(void) {
  ...
  for (port = 0; port <= BF_PLAT_MAX_QSFP; port++) {
    ...
    bf_qsfp_info_arr[port].flat_mem = false;
...
int bf_qsfp_tx_disable_single_lane(int port, int channel, bool disable) {
  ...
  rc = bf_qsfp_bitfield_rmw_onebank(
      port, CHANNEL_TX_DISABLE, (1 << channel), data);
...
int bf_qsfp_tx_disable(int port, int channel_mask, bool disable) {
  ...
  rc = bf_qsfp_bitfield_rmw(port, CHANNEL_TX_DISABLE, channel_mask, new_st);
  bf_sys_usleep(400000);
...
static int set_qsfp_idprom(int port) {
  ...
  if ((id == QSFP) || (id == QSFP_PLUS) || (id == QSFP_28)) {
    ...
    } else {  // CMIS 4.0 or newer. There was a break in backwards compatibility
