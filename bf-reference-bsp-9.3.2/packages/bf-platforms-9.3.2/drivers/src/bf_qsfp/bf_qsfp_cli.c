...
static ucli_status_t bf_pltfm_ucli_ucli__qsfpdd_summary(ucli_context_t *uc) {
  ...
  for (port = 1; port <= max_port; port++) {
    ...
    aim_printf(&uc->pvs, "   %s", bf_cmis_get_module_type_string(port));
    aim_printf(&uc->pvs, "\n");
  }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfpdd_show_for_copper(
  ...
  for (port = 1; port <= max_port; port++) {
    ...
    aim_printf(&uc->pvs,
               "  %-8s \t%s\n",
               bf_cmis_get_module_type_string(port),
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_soft_remove(ucli_context_t *uc) {
  ...
  bf_status_t sts;
  UCLI_COMMAND_INFO(uc, "soft-remove", 2, "soft-remove <port-str> <set/clear>");

  sts = pltfm_port_str_to_info(uc->pargs->args[0], &port_info);
  if (sts != BF_PLTFM_SUCCESS) {
    aim_printf(&uc->pvs,
               "Unable to get port info for str : %s : %s (%d). port-str e.g., "
               "1/0 or -/- ",
               uc->pargs->args[0],
  ...
  if (port_info.conn_id == (uint32_t)-1) {
    port_begin = 1;
    max_port = max_ports;
  } else {
    port_begin = max_port = (int)port_info.conn_id;
  }

  if (port_begin > max_ports || max_port > max_ports) {
    aim_printf(&uc->pvs, "port must between 1 and %d\n", max_ports);
    return 0;
  }
  ...
  if (strcmp(uc->pargs->args[1], "set") == 0) {
    ...
    aim_printf(
        &uc->pvs, "Usage: port-str <1/0 to %d/0> <set or clear>\n", max_ports);
    return 0;
  }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_soft_remove_show(
  ...
  bf_pltfm_port_info_t port_info;
  bf_status_t sts;
  UCLI_COMMAND_INFO(uc, "soft-remove-show", 1, " soft-remove-show <port-str>");
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_read_mul(ucli_context_t *uc) {
  ...
  if (bf_qsfp_module_read(port, QSFP_BANK0, page, offset, nBytes, val)) {
